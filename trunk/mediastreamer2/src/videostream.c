...
static void event_cb(
    void *ud, MSFilter *f, unsigned int event, void *eventdata)
{
...
static void video_steam_process_rtcp(
    VideoStream *stream, mblk_t *m)
{
    do
    {
        if (rtcp_is_SR(m))
...
static void choose_display_name(
    VideoStream *stream)
{
#ifdef WIN32
...
VideoStream *video_stream_new(
    int loc_rtp_port, int loc_rtcp_port, bool_t use_ipv6)
{
...
void video_stream_set_sent_video_size(
    VideoStream *stream, MSVideoSize vsize)
{
    if (!stream) return;
...
void video_stream_set_relay_session_id(
    VideoStream *stream, const char *id)
{
    ms_filter_call_method(stream->ms.rtpsend, MS_RTP_SEND_SET_RELAY_SESSION_ID, (void *)id);
}

void video_stream_enable_self_view(
    VideoStream *stream, bool_t val)
{
...
void video_stream_set_render_callback(
    VideoStream *s, VideoStreamRenderCallback cb, void *user_pointer)
{
    s->rendercb       = cb;
    s->render_pointer = user_pointer;
}

void video_stream_set_event_callback(
    VideoStream *s, VideoStreamEventCallback cb, void *user_pointer)
{
    s->eventcb       = cb;
    s->event_pointer = user_pointer;
}

void video_stream_set_display_filter_name(
    VideoStream *s, const char *fname)
{
...
void video_stream_set_direction(
    VideoStream *vs, VideoStreamDir dir)
{
    vs->dir = dir;
}

static MSVideoSize get_compatible_size(
    MSVideoSize maxsize, MSVideoSize wished_size)
{
...
static MSVideoSize get_with_same_orientation(
    MSVideoSize size, MSVideoSize refsize)
{
...
static void configure_video_source(
    VideoStream *stream)
{
    ...
    const char *rem_rtcp_ip, int rem_rtcp_port, int payload, int jitt_comp, MSWebCam *cam)
{
    ...
    if (stream->dir == VideoStreamSendRecv || stream->dir == VideoStreamRecvOnly)
    {
        ...
        if (stream->rendercb != NULL)
        {
            ...
            ms_filter_set_notify_callback(stream->output, (MSFilterNotifyFunc)ext_display_cb, stream);
        }
        else
        {
        ...
        ms_filter_set_notify_callback(stream->ms.decoder, (MSFilterNotifyFunc)event_cb, stream);
...
void video_stream_unprepare_video(
    VideoStream *stream)
{
...
void video_stream_update_video_params(
    VideoStream *stream)
{
...
void video_stream_change_camera(
    VideoStream *stream, MSWebCam *cam)
{
...
void video_stream_show_video(
    VideoStream *stream, bool_t show)
{
    if (stream->output)
    {
...
unsigned long video_stream_get_native_window_id(
    VideoStream *stream)
{
    unsigned long id;
    if (stream->output)
...
void video_stream_set_native_window_id(
    VideoStream *stream, unsigned long id)
{
    stream->window_id = id;
    if (stream->output)
    {
...
void video_stream_set_native_preview_window_id(
    VideoStream *stream, unsigned long id)
{
...
unsigned long video_stream_get_native_preview_window_id(
    VideoStream *stream)
{
    unsigned long id = 0;
    if (stream->output2)
...
void video_stream_use_preview_video_window(
    VideoStream *stream, bool_t yesno)
{
    stream->use_preview_window = yesno;
}

void video_stream_set_device_rotation(
    VideoStream *stream, int orientation)
{
...
void video_preview_start(
    VideoPreview *stream, MSWebCam *device)
{
    MSPixFmt    format;
...
int video_stream_send_only_start(
...
void video_stream_recv_only_stop(
    VideoStream *vs)
{
...
void video_stream_enable_display_filter_auto_rotate(
    VideoStream *stream, bool_t enable)
{
