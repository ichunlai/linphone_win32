...
static mblk_t *jpeg2yuv(
    uint8_t *jpgbuf, int bufsize, MSVideoSize *reqsize)
{
...
mblk_t *ms_load_jpeg_as_yuv(
    const char *jpgpath, MSVideoSize *reqsize)
{
#if defined(WIN32)
    mblk_t  *m = NULL;
    DWORD   st_sizel;
...
mblk_t *ms_load_nowebcam(
    MSVideoSize *reqsize, int idx)
{
    char tmp[256];
    if (idx < 0)
...
static int static_image_set_fps(
    MSFilter *f, void *arg)
{
    SIData *d = (SIData *)f->data;
    d->fps      = *((float *)arg);
...
static int static_image_get_fps(
    MSFilter *f, void *arg)
{
    SIData *d = (SIData *)f->data;
    *((float *)arg) = d->fps;
...
static int static_image_set_image(
    MSFilter *f, void *arg)
{
    SIData     *d     = (SIData *)f->data;
...
static void static_image_cam_init(
    MSWebCam *cam)
{
    cam->name = ms_strdup("Static picture");
}

static MSFilter *static_image_create_reader(
    MSWebCam *obj)
{
...
static void static_image_detect(
    MSWebCamManager *obj)
{
...
void ms_static_image_set_default_image(
    const char *path)
{
    if (def_image != NULL)
        ms_free(def_image);
