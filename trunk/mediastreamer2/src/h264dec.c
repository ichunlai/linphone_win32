...
static mblk_t *get_as_yuvmsg(
    MSFilter *f, DecData *s, AVFrame *orig)
{
...
static void update_sps(
    DecData *d, mblk_t *sps)
{
    if (d->sps)
        freemsg(d->sps);
    d->sps = dupb(sps);
...
static void update_pps(
    DecData *d, mblk_t *pps)
{
    if (d->pps)
        freemsg(d->pps);
...
static bool_t check_sps_change(
    DecData *d, mblk_t *sps)
{
    bool_t ret = FALSE;
    if (d->sps)
...
static bool_t check_pps_change(
    DecData *d, mblk_t *pps)
{
    bool_t ret = FALSE;
    if (d->pps)
...
static void enlarge_bitstream(
    DecData *d, int new_size)
{
    d->bitstream_size = new_size;
    d->bitstream      = (uint8_t *)ms_realloc(d->bitstream, d->bitstream_size);
}

static int nalusToFrame(
    DecData *d, MSQueue *naluq, bool_t *new_sps_pps)
{
    mblk_t  *im;
