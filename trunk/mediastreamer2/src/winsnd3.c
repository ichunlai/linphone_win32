...
static void winsndcard_set_level(
    MSSndCard *card, MSSndCardMixerElem e, int percent)
{
...
static int winsndcard_get_level(
    MSSndCard *card, MSSndCardMixerElem e)
{
    switch (e)
    {
    case MS_SND_CARD_MASTER:
...
static void winsndcard_set_source(
    MSSndCard *card, MSSndCardCapture source)
{
    switch (source)
    {
...
static MSSndCard *winsndcard_dup(
    MSSndCard *obj)
{
...
static MSSndCard *winsndcard_new(
    const char *name, int in_dev, int out_dev, unsigned cap)
{
    MSSndCard  *card = ms_snd_card_new(&winsnd_card_desc);
...
static void add_or_update_card(
    MSSndCardManager *m, const char *name, int indev, int outdev, unsigned int capability)
{
    MSSndCard    *card;
...
static void winsnd_apply_settings(
    WinSnd *d)
{
...
#ifndef _TRUE_TIME
static uint64_t winsnd_get_cur_time(
    void *data)
{
...
static void add_input_buffer(
    WinSnd *d, WAVEHDR *hdr, int buflen)
{
...
read_callback(
    HWAVEIN waveindev, UINT uMsg, DWORD dwInstance, DWORD dwParam1,
    DWORD dwParam2)
{
...
static void winsnd_read_preprocess(
    MSFilter *f)
{
    WinSnd   *d = (WinSnd *)f->data;
    MMRESULT mr;
...
static void winsnd_read_postprocess(
    MSFilter *f)
{
    WinSnd   *d = (WinSnd *)f->data;
    MMRESULT mr;
...
static void winsnd_read_process(
    MSFilter *f)
{
    WinSnd *d = (WinSnd *)f->data;
    mblk_t *m;
...
#if 0
    ms_warning("winsnd3 time %llu, dur %llu", GetTickCount64() - start, GetTickCount64() - pre);
    pre = GetTickCount64();
#endif
}
...
static void CALLBACK
write_callback(
    HWAVEOUT outdev, UINT uMsg, DWORD dwInstance,
...
static void winsnd_write_preprocess(
    MSFilter *f)
{
    WinSnd   *d = (WinSnd *)f->data;
    MMRESULT mr;
...
static void playout_buf(
    WinSnd *d, WAVEHDR *hdr, mblk_t *m)
{
    MMRESULT mr;
    hdr->dwUser         = (DWORD)m;
...
static void winsnd_write_process(
    MSFilter *f)
{
    WinSnd   *d = (WinSnd *)f->data;
    mblk_t   *m;
...
static int winsnd_get_stat_input(
    MSFilter *f, void *arg)
{
    WinSnd *d = (WinSnd *)f->data;
    return d->stat_input;
...
static int winsnd_get_stat_ouptut(
    MSFilter *f, void *arg)
{
    WinSnd *d = (WinSnd *)f->data;
...
static int winsnd_get_stat_discarded(
    MSFilter *f, void *arg)
{
    WinSnd *d = (WinSnd *)f->data;
