...
static int compare_stats_with_name(
    const MSFilterStats *stat, const char *name)
{
    return strcmp(stat->name, name);
}

static MSFilterStats *find_or_create_stats(
    MSFilterDesc *desc)
{
    MSList        *elem = ms_list_find_custom(stats_list, (MSCompareFunc)compare_stats_with_name, (void *)desc->name);
...
bool_t ms_filter_codec_supported(
    const char *mime)
{
...
MSFilterDesc *ms_filter_get_encoding_capturer(
    const char *mime)
{
    MSList *elem;
...
MSFilterDesc *ms_filter_get_decoding_renderer(
    const char *mime)
{
    MSList *elem;
...
MSFilterDesc *ms_filter_get_encoder(
    const char *mime)
{
    MSList *elem;
    for (elem = desc_list; elem != NULL; elem = ms_list_next(elem))
...
MSFilterDesc *ms_filter_get_decoder(
    const char *mime)
{
    MSList *elem;
    for (elem = desc_list; elem != NULL; elem = ms_list_next(elem))
...
MSFilter *ms_filter_create_encoder(
    const char *mime)
{
...
MSFilter *ms_filter_create_decoder(
    const char *mime)
{
...
MSFilter *ms_filter_new_from_desc(
    MSFilterDesc *desc)
{
    MSFilter *obj;
...
MSFilterDesc *ms_filter_lookup_by_name(
...
MSFilter *ms_filter_new_from_name(
    const char *filter_name)
{
...
int ms_filter_link(
    MSFilter *f1, int pin1, MSFilter *f2, int pin2)
{
    MSQueue *q;
...
static inline bool_t is_interface_method(
    unsigned int magic)
{
...
bool_t ms_filter_has_method(
    MSFilter *f, unsigned int id)
{
    MSFilterMethod *methods = f->desc->methods;
    int            i;
    for (i = 0; methods != NULL && methods[i].method != NULL; i++)
...
int ms_filter_call_method_noarg(
    MSFilter *f, unsigned int id)
{
    return ms_filter_call_method(f, id, NULL);
}

void ms_filter_set_notify_callback(
    MSFilter *f, MSFilterNotifyFunc fn, void *ud)
{
    f->notify    = fn;
    f->notify_ud = ud;
}

void ms_filter_destroy(
    MSFilter *f)
{
...
void ms_filter_preprocess(
    MSFilter *f, struct _MSTicker *t)
{
    f->last_tick = 0;
    f->ticker    = t;
...
bool_t ms_filter_inputs_have_data(
    MSFilter *f)
{
    int i;
...
void ms_filter_postpone_task(
    MSFilter *f, MSFilterFunc taskfunc)
{
    MSFilterTask *task;
...
MSList *ms_filter_find_neighbours(
    MSFilter *me)
{
    MSList *l = NULL;
    MSList *it;
...
void ms_connection_helper_start(
...
int ms_connection_helper_link(
    MSConnectionHelper *h, MSFilter *f, int inpin, int outpin)
{
    int err = 0;
...
int ms_connection_helper_unlink(
    MSConnectionHelper *h, MSFilter *f, int inpin, int outpin)
{
    int err = 0;
    if (h->last.filter == NULL)
...
void ms_filter_enable_statistics(
    bool_t enabled)
{
    statistics_enabled = enabled;
}

const MSList *ms_filter_get_statistics(
    void)
{
    return stats_list;
}

void ms_filter_reset_statistics(
    void)
{
    MSList *elem;
...
static int usage_compare(
    const MSFilterStats *s1, const MSFilterStats *s2)
{
