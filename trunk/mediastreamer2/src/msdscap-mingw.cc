...
public:
    ...
    ComPtr(
        const ComPtr<_ComT>& other) : mPtr(other.mPtr)
    {
        if (mPtr) mPtr->AddRef();
    }

    ComPtr<_ComT> & operator=(
        const ComPtr<_ComT> &other)
    {
        if (other.mPtr)
            other.mPtr->AddRef();
        if (mPtr)
...
ComPtr< IPin > getPin(
    IBaseFilter *filter, PIN_DIRECTION direction, int num)
{
    ComPtr< IPin >      retVal;
...
STDMETHODIMP DSCapture::QueryInterface(
...
STDMETHODIMP DSCapture::SampleCB(
    double par1, IMediaSample *sample)
{
    uint8_t      *p;
    unsigned int size;
...
STDMETHODIMP DSCapture::BufferCB(
    double, BYTE *b, long len)
{
    ms_message("DSCapture::BufferCB");
    return S_OK;
}

static void dscap_init(
    MSFilter *f)
{
...
static char *fourcc_to_char(
    char *str, uint32_t fcc)
{
    memcpy(str, &fcc, 4);
    str[4] = '\0';
    return str;
...
static int find_best_format(
    ComPtr<IAMStreamConfig> streamConfig, int count, MSVideoSize *requested_size, MSPixFmt requested_fmt)
{
    int         i;
...
int DSCapture::selectBestFormat(
    ComPtr<IAMStreamConfig> streamConfig, int count)
{
    int index;
    _pixfmt = MS_YUV420P;
...
bool DSCapture::isTimeToSend(
    uint64_t ticker_time)
{
    if (_frame_count == -1)
    {
...
static MSFilter *ms_dshow_create_reader(
    MSWebCam *obj)
{
