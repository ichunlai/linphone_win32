...
/*
static AudioInputLevelChanged volume_input_leve_callback = NULL;
static AudioOutputLevelChanged volume_output_leve_callback = NULL;
*/
static float input_level = 0.0f;
static float output_level = 0.0f;
/*
void ms_snd_card_set_volume_callback(AudioInputLevelChanged input,AudioOutputLevelChanged output)
{
	if(input)
		volume_input_leve_callback = input;
	if(output)
		volume_output_leve_callback = output;
}*/
...
long usecdiff(struct timeval * t0, struct timeval * t1)
{
...
int
rtp_gettimeofday (struct timeval *tp, void *tz)
{
	struct _timeb timebuffer;

	_ftime (&timebuffer);
	tp->tv_sec = timebuffer.time;
	tp->tv_usec = (long)(timebuffer.millitm) * (1000000/1000);
	return 0;
}
...
void do_level_callback()
{
	...
	rtp_gettimeofday(&now, 0);

	if ( last.tv_sec != 0 && usecdiff(&now, &last) < 60 )
		return;

	last = now;

	//获取音量
	getinputLevel =   vol_to_db(input_level);
	getoutputLevel =  vol_to_db(output_level);



	inputLevel = (int) getinputLevel;
	...
	outputLevel = (int) getoutputLevel;
...
//		if(volume_input_leve_callback != NULL)
//			volume_input_leve_callback(inputLevel);
	}


	if(lastOutputLevel != outputLevel) {
		lastOutputLevel =  outputLevel;
//		if(volume_output_leve_callback != NULL)
//			volume_output_leve_callback(outputLevel);
	}

	//ms_error("do_level_callback: input %d, output %d",inputLevel,outputLevel);
...
#else
			add_or_update_card(m,incaps.szPname,item,-2,MS_SND_CARD_CAP_CAPTURE);
#endif
		}
...
#else
			add_or_update_card(m,outcaps.szPname,-2,item,MS_SND_CARD_CAP_PLAYBACK);
#endif
		}
	}
}
