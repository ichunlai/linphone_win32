 ...
 */
static void media_stream_change_decoder(
    MediaStream *stream, int payload)
{
...
MSTickerPrio __ms_get_default_prio(
    bool_t is_video)
{
    const char *penv;
...
RtpSession *create_duplex_rtpsession(
...
void start_ticker(
    MediaStream *stream)
{
    MSTickerParams params = { MS_TICKER_PRIO_NORMAL };
    char           name[16];
...
const char *media_stream_type_str(
    MediaStream *stream)
{
    switch (stream->type)
    {
...
void media_stream_set_rtcp_information(
    MediaStream *stream, const char *cname, const char *tool)
{
    if (stream->session != NULL)
    {
...
void media_stream_get_local_rtp_stats(
    MediaStream *stream, rtp_stats_t *lstats)
{
    if (stream->session)
    {
...
int media_stream_set_dscp(
    MediaStream *stream, int dscp)
{
...
void media_stream_enable_adaptive_bitrate_control(
    MediaStream *stream, bool_t enabled)
{
    stream->use_rc = enabled;
}

void media_stream_enable_adaptive_jittcomp(
    MediaStream *stream, bool_t enabled)
{
    rtp_session_enable_adaptive_jitter_compensation(stream->session, enabled);
}

bool_t media_stream_enable_srtp(
    MediaStream *stream, enum ortp_srtp_crypto_suite_t suite, const char *snd_key, const char *rcv_key)
{
...
const MSQualityIndicator *media_stream_get_quality_indicator(
    MediaStream *stream)
{
    return stream->qi;
}
...
void mediastream_payload_type_changed(
    RtpSession *session, unsigned long data)
{
...
float media_stream_get_quality_rating(
    MediaStream *stream)
{
    if (stream->qi)
    {
...
float media_stream_get_average_quality_rating(
    MediaStream *stream)
{
    if (stream->qi)
    {
