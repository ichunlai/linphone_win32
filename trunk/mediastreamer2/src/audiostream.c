...
static void on_dtmf_received(
    RtpSession *s, int dtmf, void *user_data)
{
...
static void audio_stream_configure_resampler(
    MSFilter *resampler, MSFilter *from, MSFilter *to)
{
...
static void audio_stream_process_rtcp(
    AudioStream *stream, mblk_t *m)
{
    do
    {
...
bool_t audio_stream_alive(
    AudioStream *stream, int timeout)
{
...
static mblk_t *audio_stream_payload_picker(
    MSRtpPayloadPickerContext *context, unsigned int sequence_number)
{
    return rtp_session_pick_with_cseq(((AudioStream *)(context->filter_graph_manager))->ms.session, sequence_number);
}

static void stop_preload_graph(
    AudioStream *stream)
{
...
void audio_stream_prepare_sound(
    AudioStream *stream, MSSndCard *playcard, MSSndCard *captcard)
{
...
void audio_stream_unprepare_sound(
    AudioStream *stream)
{
    if (stream->dummy)
    {
...
int audio_stream_start_with_files(
    AudioStream *stream, RtpProfile *prof, const char *remip, int remport,
...
AudioStream *audio_stream_start(
    RtpProfile *prof, int locport, const char *remip, int remport, int profile, int jitt_comp, bool_t use_ec)
{
...
AudioStream *audio_stream_start_with_sndcards(
    RtpProfile *prof, int locport, const char *remip, int remport, int profile, int jitt_comp, MSSndCard *playcard, MSSndCard *captcard, bool_t use_ec)
{
    AudioStream *stream;
    if (playcard == NULL)
    {
...
void audio_stream_play(
    AudioStream *st, const char *name)
{
...
int audio_stream_mixed_record_open(
    AudioStream *st, const char *filename)
{
...
int audio_stream_mixed_record_start(
    AudioStream *st)
{
...
int audio_stream_mixed_record_stop(
    AudioStream *st)
{
    if (st->recorder && st->recorder_file)
    {
...
uint32_t audio_stream_get_features(
    AudioStream *st)
{
    return st->features;
}

void audio_stream_set_features(
    AudioStream *st, uint32_t features)
{
    st->features = features;
}

AudioStream *audio_stream_new(
    int loc_rtp_port, int loc_rtcp_port, bool_t ipv6)
{
...
void audio_stream_play_received_dtmfs(
    AudioStream *st, bool_t yesno)
{
    st->play_dtmfs = yesno;
}

int audio_stream_start_now(
    AudioStream *stream, RtpProfile *prof,  const char *remip, int remport, int rem_rtcp_port, int payload_type, int jitt_comp, MSSndCard *playcard, MSSndCard *captcard, bool_t use_ec)
{
...
void audio_stream_set_relay_session_id(
    AudioStream *stream, const char *id)
{
    ms_filter_call_method(stream->ms.rtpsend, MS_RTP_SEND_SET_RELAY_SESSION_ID, (void *)id);
}

void audio_stream_set_echo_canceller_params(
    AudioStream *stream, int tail_len_ms, int delay_ms, int framesize)
{
    if (stream->ec)
    {
        if (tail_len_ms > 0)
...
void audio_stream_enable_echo_limiter(
    AudioStream *stream, EchoLimiterType type)
{
    stream->el_type = type;
    if (stream->volsend)
    {
...
void audio_stream_enable_gain_control(
    AudioStream *stream, bool_t val)
{
    stream->use_gc = val;
}

void audio_stream_enable_automatic_gain_control(
    AudioStream *stream, bool_t val)
{
    stream->use_agc = val;
}

void audio_stream_enable_noise_gate(
    AudioStream *stream, bool_t val)
{
    stream->use_ng = val;
    if (stream->volsend)
...
void audio_stream_set_mic_gain(
    AudioStream *stream, float gain)
{
    if (stream->volsend)
    {
...
void audio_stream_enable_equalizer(
...
void audio_stream_equalizer_set_gain(
    AudioStream *stream, int frequency, float gain, int freq_width)
{
    if (stream->equalizer)
    {
        MSEqualizerGain d;
...
int audio_stream_send_dtmf(
    AudioStream *stream, char dtmf)
{
    if (stream->dtmfgen_rtp)
...
float audio_stream_get_quality_rating(
    AudioStream *stream)
{
    return media_stream_get_quality_rating(&stream->ms);
}

float audio_stream_get_average_quality_rating(
    AudioStream *stream)
{
    return media_stream_get_average_quality_rating(&stream->ms);
}

void audio_stream_enable_zrtp(
    AudioStream *stream, OrtpZrtpParams *params)
{
