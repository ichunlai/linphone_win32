    ...
    MSFilter *f)
{
    ...
    ms_filter_unlock(f);
#if 0
    ms_warning("%s: time %llu, dur %llu", f->desc->name, GetTickCount64() - start, GetTickCount64() - pre);
    pre = GetTickCount64();
#endif
}

static void enc_postprocess(
    MSFilter *f)
{
