...
static uint64_t generate_tie_breaker(
    void)
{
...
static char *generate_pwd(
    void)
{
    char *pwd = (char *)ms_malloc(25);
    sprintf(pwd, "%08lx%08lx%08lx", random(), random(), random());
    pwd[24] = '\0';
    return pwd;
}

static void ice_session_init(
    IceSession *session)
{
...
static void ice_check_list_init(
    IceCheckList *cl)
{
    cl->session                  = NULL;
    cl->rtp_session              = NULL;
...
IceCheckList *ice_check_list_new(
    void)
{
...
static void ice_compute_pair_priority(
    IceCandidatePair *pair, const IceRole *role)
{
...
static IceCandidatePair *ice_pair_new(
...
static void ice_free_stun_server_check_transaction(
    IceStunServerCheckTransaction *transaction)
{
    ms_free(transaction);
}

static void ice_free_stun_server_check(
...
static void ice_free_transaction(
    IceTransaction *transaction)
{
    ms_free(transaction);
}

static void ice_free_pair_foundation(
    IcePairFoundation *foundation)
{
    ms_free(foundation);
}

static void ice_free_valid_pair(
    IceValidCandidatePair *valid_pair)
{
    ms_free(valid_pair);
}

static void ice_free_candidate_pair(
    IceCandidatePair *pair, IceCheckList *cl)
{
    MSList *elem;
    ...
    {
        ice_free_valid_pair((IceValidCandidatePair *)(elem->data));
        cl->valid_list = ms_list_remove_link(cl->valid_list, elem);
    }
    ms_free(pair);
}

static void ice_free_candidate(
...
static void ice_pair_set_state(
    IceCandidatePair *pair, IceCandidatePairState state)
{
    if (pair->state != state)
    {
        pair->state = state;
...
IceCheckListState ice_check_list_state(
    const IceCheckList *cl)
{
    return cl->state;
}

static int ice_find_check_list_from_state(
    const IceCheckList *cl, const IceCheckListState *state)
{
    return (cl->state != *state);
}

void ice_check_list_set_state(
    IceCheckList *cl, IceCheckListState state)
{
...
const char *ice_check_list_remote_pwd(
    const IceCheckList *cl)
{
...
static int ice_find_default_local_candidate(
    const IceCandidate *candidate, const uint16_t *componentID)
{
    return !((candidate->componentID == *componentID) && (candidate->is_default == TRUE));
}

bool_t ice_check_list_remote_credentials_changed(
    IceCheckList *cl, const char *ufrag, const char *pwd)
{
...
void ice_check_list_set_remote_credentials(
    IceCheckList *cl, const char *ufrag, const char *pwd)
{
    ice_set_credentials(&cl->remote_ufrag, &cl->remote_pwd, ufrag, pwd);
}

bool_t ice_check_list_default_local_candidate(
    const IceCheckList *cl, const char **rtp_addr, int *rtp_port, const char **rtcp_addr, int *rtcp_port)
{
...
bool_t ice_check_list_selected_valid_local_candidate(
    const IceCheckList *cl, const char **rtp_addr, int *rtp_port, const char **rtcp_addr, int *rtcp_port)
{
    IceCandidate          *candidate  = NULL;
...
bool_t ice_check_list_selected_valid_remote_candidate(
    const IceCheckList *cl, const char **rtp_addr, int *rtp_port, const char **rtcp_addr, int *rtcp_port)
{
    IceCandidate          *candidate  = NULL;
    IceValidCandidatePair *valid_pair = NULL;
...
IceCandidateType ice_check_list_selected_valid_candidate_type(
    const IceCheckList *cl)
{
    MSList   *elem;
...
void ice_check_list_check_completed(
    IceCheckList *cl)
{
    CheckList_Bool cb;
...
static void ice_check_list_queue_triggered_check(
    IceCheckList *cl, IceCandidatePair *pair)
{
...
static IceCandidatePair *ice_check_list_pop_triggered_check(
    IceCheckList *cl)
{
    IceCandidatePair *pair;

    if (ms_list_size(cl->triggered_checks_queue) == 0) return NULL;
    pair = (IceCandidatePair *)ms_list_nth_data(cl->triggered_checks_queue, 0);
    if (pair != NULL)
    {
...
static int ice_find_non_frozen_pair(
    const IceCandidatePair *pair, const void *dummy)
{
    return (pair->state == ICP_Frozen);
}

static bool_t ice_check_list_is_frozen(
    const IceCheckList *cl)
{
...
bool_t ice_check_list_is_mismatch(
    const IceCheckList *cl)
{
    return cl->mismatch;
}
...
IceCheckList *ice_session_check_list(
    const IceSession *session, unsigned int n)
{
...
static void ice_session_compute_pair_priorities(
    IceSession *session)
{
    ms_list_for_each(session->streams, (void (*)(void *))ice_check_list_compute_pair_priorities);
}

IceSessionState ice_session_state(
    const IceSession *session)
{
    return session->state;
}

IceRole ice_session_role(
    const IceSession *session)
{
    return session->role;
}

void ice_session_set_role(
    IceSession *session, IceRole role)
{
...
void ice_session_set_local_credentials(
    IceSession *session, const char *ufrag, const char *pwd)
{
    ice_set_credentials(&session->local_ufrag, &session->local_pwd, ufrag, pwd);
}

bool_t ice_session_remote_credentials_changed(
    IceSession *session, const char *ufrag, const char *pwd)
{
...
void ice_session_set_remote_credentials(
    IceSession *session, const char *ufrag, const char *pwd)
{
    ice_set_credentials(&session->remote_ufrag, &session->remote_pwd, ufrag, pwd);
}

void ice_session_set_max_connectivity_checks(
    IceSession *session, uint8_t max_connectivity_checks)
{
    session->max_connectivity_checks = max_connectivity_checks;
}

void ice_session_set_keepalive_timeout(
    IceSession *session, uint8_t timeout)
{
...
int ice_session_nb_check_lists(
    IceSession *session)
{
    return ms_list_size(session->streams);
}

static int ice_find_completed_check_list(
    const IceCheckList *cl, const void *dummy)
{
    return (cl->state != ICL_Completed);
}

bool_t ice_session_has_completed_check_list(
    const IceSession *session)
{
...
void ice_session_add_check_list(
    IceSession *session, IceCheckList *cl)
{
...
void ice_session_remove_check_list(
    IceSession *session, IceCheckList *cl)
{
    if (cl == NULL) return;
...
static int ice_find_default_candidate_from_componentID(
    const IceCandidate *candidate, const uint16_t *componentID)
{
    return !((candidate->is_default == TRUE) && (candidate->componentID == *componentID));
}

static void ice_find_default_remote_candidate_for_componentID(
    const uint16_t *componentID, IceCheckList *cl)
{
    MSList *elem = ms_list_find_custom(cl->remote_candidates, (MSCompareFunc)ice_find_default_candidate_from_componentID, (void *)componentID);
    if (elem == NULL)
    {
        cl->mismatch = TRUE;
        cl->state    = ICL_Failed;
...
static void ice_check_list_candidates_gathered_result_ptr(
    const IceCheckList *cl, bool_t *result)
{
    if (cl->gathering_finished == FALSE) *result = FALSE;
}

bool_t ice_check_list_candidates_gathered(
    const IceCheckList *cl)
{
    return cl->gathering_finished;
}

bool_t ice_session_candidates_gathered(
    const IceSession *session)
{
    bool_t result = TRUE;
...
static void ice_check_list_gathering_needed(
    const IceCheckList *cl, bool_t *gathering_needed)
{
    if (cl->gathering_finished == FALSE) *gathering_needed = TRUE;
}

static void ice_check_list_gather_candidates(
    IceCheckList *cl, Session_Index *si)
{
    IceStunServerCheck *check;
...
void ice_session_gather_candidates(
    IceSession *session, struct sockaddr_storage ss, socklen_t ss_len)
{
    Session_Index si;
    OrtpEvent     *ev;
...
int ice_session_gathering_duration(
    IceSession *session)
{
...
static void ice_transaction_sum_gathering_round_trip_time(
    const IceStunServerCheckTransaction *transaction, StunRequestRoundTripTime *rtt)
{
...
static void ice_stun_server_check_sum_gathering_round_trip_time(
    const IceStunServerCheck *check, StunRequestRoundTripTime *rtt)
{
    ms_list_for_each2(check->transactions, (void (*)(void *, void *))ice_transaction_sum_gathering_round_trip_time, rtt);
}

static void ice_check_list_sum_gathering_round_trip_times(
    const IceCheckList *cl, StunRequestRoundTripTime *rtt)
{
    ms_list_for_each2(cl->stun_server_checks, (void (*)(void *, void *))ice_stun_server_check_sum_gathering_round_trip_time, rtt);
}

int ice_session_average_gathering_round_trip_time(
    IceSession *session)
{
...
static void ice_unselect_valid_pair(
    IceValidCandidatePair *valid_pair)
{
    valid_pair->selected = FALSE;
}

static void ice_check_list_select_candidates(
    IceCheckList *cl)
{
    IceValidCandidatePair *valid_pair = NULL;
    uint16_t              componentID;
...
void ice_session_select_candidates(
    IceSession *session)
{
...
static IceTransaction *ice_create_transaction(
    IceCheckList *cl, IceCandidatePair *pair, const UInt96 *tr_id)
{
...
static int ice_find_transaction_from_pair(
    const IceTransaction *transaction, const IceCandidatePair *pair)
{
    return (transaction->pair != pair);
}

static IceTransaction *ice_find_transaction(
    const IceCheckList *cl, const IceCandidatePair *pair)
{
    MSList *elem = ms_list_find_custom(cl->transaction_list, (MSCompareFunc)ice_find_transaction_from_pair, (void *)pair);
...
static void ice_send_stun_server_binding_request(
    ortp_socket_t sock, const struct sockaddr *server, socklen_t addrlen, IceStunServerCheck *check)
{
    StunMessage              msg;
...
static int ice_parse_stun_server_binding_response(
    const StunMessage *msg, char *addr, int addr_len, int *port)
{
...
static void ice_send_binding_request(
    IceCheckList *cl, IceCandidatePair *pair, const RtpSession *rtp_session)
{
    StunMessage    msg;
    StunAddress4   dest;
    StunAtrString  username;
...
static int ice_get_componentID_from_rtp_session(
...
static int ice_get_socket_from_rtp_session(
    const RtpSession *rtp_session, const OrtpEventData *evt_data)
{
    if (evt_data->info.socket_type == OrtpRTPSocket)
    {
...
static int ice_get_recv_port_from_rtp_session(
    const RtpSession *rtp_session, const OrtpEventData *evt_data)
{
    if (evt_data->info.socket_type == OrtpRTPSocket)
    {
...
static void ice_send_binding_response(
    const RtpSession *rtp_session, const OrtpEventData *evt_data, const StunMessage *msg, const StunAddress4 *dest)
{
...
static void ice_send_error_response(
    const RtpSession *rtp_session, const OrtpEventData *evt_data, const StunMessage *msg, uint8_t err_class, uint8_t err_num, const StunAddress4 *dest, const char *error)
{
    StunMessage        response;
    StunAtrString      password;
    char               buf[STUN_MAX_MESSAGE_SIZE];
...
static void ice_send_indication(
    const IceCandidatePair *pair, const RtpSession *rtp_session)
{
...
static void ice_send_keepalive_packet_for_componentID(
    const uint16_t *componentID, const CheckList_RtpSession *cr)
{
    MSList *elem = ms_list_find_custom(cr->cl->valid_list, (MSCompareFunc)ice_find_selected_valid_pair_from_componentID, (void *)componentID);
    if (elem != NULL)
    {
...
static void ice_send_keepalive_packets(
    IceCheckList *cl, const RtpSession *rtp_session)
{
...
static int ice_find_candidate_from_transport_address(
    const IceCandidate *candidate, const IceTransportAddress *taddr)
{
    return ice_compare_transport_addresses(&candidate->taddr, taddr);
}

static int ice_find_candidate_from_ip_address(
    const IceCandidate *candidate, const char *ipaddr)
{
...
static int ice_check_received_binding_request_attributes(
    const RtpSession *rtp_session, const OrtpEventData *evt_data, const StunMessage *msg, const StunAddress4 *remote_addr)
{
...
static int ice_check_received_binding_request_integrity(
    const IceCheckList *cl, const RtpSession *rtp_session, const OrtpEventData *evt_data, const StunMessage *msg, const StunAddress4 *remote_addr)
{
    char     hmac[20];
...
static int ice_check_received_binding_request_username(
    const IceCheckList *cl, const RtpSession *rtp_session, const OrtpEventData *evt_data, const StunMessage *msg, const StunAddress4 *remote_addr)
{
    char username[256];
    char *colon;
...
static int ice_check_received_binding_request_role_conflict(
    const IceCheckList *cl, const RtpSession *rtp_session, const OrtpEventData *evt_data, const StunMessage *msg, const StunAddress4 *remote_addr)
{
...
static void ice_fill_transport_address(
    IceTransportAddress *taddr, const char *ip, int port)
{
...
static int ice_find_candidate_from_foundation(
    const IceCandidate *candidate, const char *foundation)
{
    return !((strlen(candidate->foundation) == strlen(foundation)) && (strcmp(candidate->foundation, foundation) == 0));
}

static void ice_generate_arbitrary_foundation(
    char *foundation, int len, MSList *list)
{
    uint64_t r;
    MSList   *elem;
...
static IceCandidate *ice_learn_peer_reflexive_candidate(
    IceCheckList *cl, const OrtpEventData *evt_data, const StunMessage *msg, const IceTransportAddress *taddr)
{
    char         foundation[32];
    IceCandidate *candidate = NULL;
    MSList       *elem;
    ...
    elem        = ms_list_find_custom(cl->remote_candidates, (MSCompareFunc)ice_find_candidate_from_transport_address, (void *)taddr);
    if (elem == NULL)
    {
...
static int ice_find_pair_from_candidates(
    const IceCandidatePair *pair, const LocalCandidate_RemoteCandidate *candidates)
{
    return !((pair->local == candidates->local) && (pair->remote == candidates->remote));
}

/* Trigger checks as defined in 7.2.1.4. */
static IceCandidatePair *ice_trigger_connectivity_check_on_binding_request(
    IceCheckList *cl, const RtpSession *rtp_session, const OrtpEventData *evt_data, IceCandidate *prflx_candidate, const IceTransportAddress *remote_taddr)
{
    ...
    {
        elem = ms_list_find_custom(cl->remote_candidates, (MSCompareFunc)ice_find_candidate_from_transport_address, (void *)remote_taddr);
        if (elem == NULL)
        {
...
static void ice_update_nominated_flag_on_binding_request(
    const IceCheckList *cl, const StunMessage *msg, IceCandidatePair *pair)
{
...
static void ice_handle_received_binding_request(
    IceCheckList *cl, RtpSession *rtp_session, const OrtpEventData *evt_data, const StunMessage *msg, const StunAddress4 *remote_addr, const char *src6host)
{
...
static int ice_find_stun_server_check(
    const IceStunServerCheck *check, const ortp_socket_t *sock)
{
    return !(check->sock == *sock);
}

static int ice_find_check_list_gathering_candidates(
    const IceCheckList *cl, const void *dummy)
{
    return (cl->gathering_candidates == FALSE);
}

static int ice_find_pair_from_transactionID(
    const IceTransaction *transaction, const UInt96 *transactionID)
{
    return memcmp(&transaction->transactionID, transactionID, sizeof(transaction->transactionID));
}

static int ice_check_received_binding_response_addresses(
    const RtpSession *rtp_session, const OrtpEventData *evt_data, IceCandidatePair *pair, const StunAddress4 *remote_addr)
{
    StunAddress4 dest;
    StunAddress4 local;
    int          recvport = ice_get_recv_port_from_rtp_session(rtp_session, evt_data);
...
static int ice_check_received_binding_response_attributes(
    const StunMessage *msg, const StunAddress4 *remote_addr)
{
    if (!msg->hasUsername)
    {
...
static IceCandidate *ice_discover_peer_reflexive_candidate(
    IceCheckList *cl, const IceCandidatePair *pair, const StunMessage *msg)
{
    struct sockaddr_in  addr_in;
    IceTransportAddress taddr;
    IceCandidate        *candidate = NULL;
...
static int ice_compare_valid_pair_priorities(
    const IceValidCandidatePair *vp1, const IceValidCandidatePair *vp2)
{
    return ice_compare_pair_priorities(vp1->valid, vp2->valid);
}

static int ice_find_valid_pair(
    const IceValidCandidatePair *vp1, const IceValidCandidatePair *vp2)
{
...
static IceCandidatePair *ice_construct_valid_pair(
...
static int ice_compare_pair_foundations(
    const IceCandidatePair *p1, const IceCandidatePair *p2)
{
...
static void ice_change_state_of_frozen_pairs_to_waiting(
    IceCandidatePair *pair, const IceCandidatePair *succeeded_pair)
{
...
static void ice_update_pair_states_on_binding_response(
    IceCheckList *cl, IceCandidatePair *pair)
{
...
static void ice_update_nominated_flag_on_binding_response(
    const IceCheckList *cl, IceCandidatePair *valid_pair, const IceCandidatePair *succeeded_pair, IceCandidatePairState succeeded_pair_previous_state)
{
    switch (cl->session->role)
    {
    case IR_Controlling:
...
static int ice_find_not_failed_or_succeeded_pair(
    const IceCandidatePair *pair, const void *dummy)
{
    return !((pair->state != ICP_Failed) && (pair->state != ICP_Succeeded));
}

static int ice_compare_transactionIDs(
    const UInt96 *tr_id1, const UInt96 *tr_id2)
{
    return memcmp(tr_id1, tr_id2, sizeof(UInt96));
}

static int ice_find_non_responded_stun_server_check(
    const IceStunServerCheck *check, const void *dummy)
{
    return (check->responded == TRUE);
}

static void ice_handle_received_binding_response(
    IceCheckList *cl, RtpSession *rtp_session, const OrtpEventData *evt_data, const StunMessage *msg, const StunAddress4 *remote_addr)
{
    ...
    if (cl->gathering_candidates == TRUE)
    {
        if ((htonl(remote_addr->addr) == servaddr->sin_addr.s_addr) && (htons(remote_addr->port) == servaddr->sin_port))
        {
            ...
            if (elem != NULL)
            {
                ...
                elem = ms_list_find_custom(check->transactions, (MSCompareFunc) ice_compare_transactionIDs, (void *)&msg->msgHdr.tr_id);
                if (elem != NULL)
                {
                    ...
                    if (transaction != NULL)
                    {
                        ...
                        {
                            base_elem = ms_list_find_custom(cl->local_candidates, (MSCompareFunc)ice_find_host_candidate, (void *)&componentID);
                            if (base_elem != NULL)
                            {
    ...
    elem = ms_list_find_custom(cl->transaction_list, (MSCompareFunc)ice_find_pair_from_transactionID, (void *)&msg->msgHdr.tr_id);
    if (elem == NULL)
    {
...
static void ice_handle_received_error_response(
    IceCheckList *cl, RtpSession *rtp_session, const StunMessage *msg)
{
    IceCandidatePair *pair;
    MSList           *elem = ms_list_find_custom(cl->transaction_list, (MSCompareFunc)ice_find_pair_from_transactionID, (void *)&msg->msgHdr.tr_id);
    if (elem == NULL)
    {
        /* We received an error response concerning an unknown binding request, ignore it... */
        return;
...
void ice_handle_stun_packet(
    IceCheckList *cl, RtpSession *rtp_session, const OrtpEventData *evt_data)
{
    StunMessage             msg;
...
static IceCandidate *ice_candidate_new(
    const char *type, const char *ip, int port, uint16_t componentID)
{
    IceCandidate     *candidate;
...
static void ice_compute_candidate_priority(
    IceCandidate *candidate)
{
...
static int ice_find_componentID(
    const uint16_t *cid1, const uint16_t *cid2)
{
    return !(*cid1 == *cid2);
}

static void ice_add_componentID(
    MSList **list, uint16_t *componentID)
{
...
IceCandidate *ice_add_local_candidate(
    IceCheckList *cl, const char *type, const char *ip, int port, uint16_t componentID, IceCandidate *base)
{
    MSList       *elem;
    IceCandidate *candidate;
...
IceCandidate *ice_add_remote_candidate(
    IceCheckList *cl, const char *type, const char *ip, int port, uint16_t componentID, uint32_t priority, const char *const foundation, bool_t is_default)
{
    MSList       *elem;
    IceCandidate *candidate;
...
static int ice_find_pair_in_valid_list(
    IceValidCandidatePair *valid_pair, IceCandidatePair *pair)
{
    return (valid_pair->valid != pair);
}

static void ice_check_if_losing_pair_should_cause_restart(
    const IceCandidatePair *pair, LosingRemoteCandidate_InProgress_Failed *lif)
{
...
void ice_add_losing_pair(
    IceCheckList *cl, uint16_t componentID, const char *local_addr, int local_port, const char *remote_addr, int remote_port)
{
    IceTransportAddress            taddr;
    Type_ComponentID               tc;
    MSList                         *elem;
    ...
    if (elem == NULL)
    {
        ...
        elem = ms_list_find_custom(cl->remote_candidates, (MSCompareFunc)ice_find_candidate_from_ip_address, (void *)local_addr);
        if (elem != NULL)
        {
...
static void ice_check_list_has_losing_pairs(
    const IceCheckList *cl, int *nb_losing_pairs)
{
    *nb_losing_pairs += ms_list_size(cl->losing_pairs);
}

int ice_session_nb_losing_pairs(
    const IceSession *session)
{
...
static int ice_find_candidate_with_same_foundation(
    const IceCandidate *c1, const IceCandidate *c2)
{
...
static void ice_compute_candidate_foundation(
    IceCandidate *candidate, IceCheckList *cl)
{
...
static void ice_check_list_compute_candidates_foundations(
    IceCheckList *cl)
{
    if (cl->state == ICL_Running)
    {
...
static int ice_find_redundant_candidate(
    const IceCandidate *c1, const IceCandidate *c2)
{
    if (c1 == c2) return 1;
...
static void ice_check_list_eliminate_redundant_candidates(
    IceCheckList *cl)
{
    MSList       *elem;
    MSList       *other_elem;
...
void ice_session_eliminate_redundant_candidates(
    IceSession *session)
{
...
static int ice_find_candidate_from_type_and_componentID(
    const IceCandidate *candidate, const Type_ComponentID *tc)
{
    return !((candidate->type == tc->type) && (candidate->componentID == tc->componentID));
}

static void ice_choose_local_or_remote_default_candidates(
    IceCheckList *cl, MSList *list)
{
    Type_ComponentID tc;
    MSList           *l;
    int              i;
...
static void ice_check_list_choose_default_candidates(
    IceCheckList *cl)
{
    if (cl->state == ICL_Running)
    {
...
static int ice_compare_pair_priorities(
    const IceCandidatePair *p1, const IceCandidatePair *p2)
{
...
static void ice_form_candidate_pairs(
    IceCheckList *cl)
{
...
static void ice_replace_srflx_by_base_in_pair(
    IceCandidatePair *pair)
{
...
static int ice_compare_transport_addresses(
    const IceTransportAddress *ta1, const IceTransportAddress *ta2)
{
...
static int ice_compare_candidates(
    const IceCandidate *c1, const IceCandidate *c2)
{
...
static int ice_prune_duplicate_pair(
    IceCandidatePair *pair, MSList **pairs, IceCheckList *cl)
{
...
static void ice_create_check_list(
    IceCandidatePair *pair, IceCheckList *cl)
{
    cl->check_list = ms_list_insert_sorted(cl->check_list, pair, (MSCompareFunc)ice_compare_pair_priorities);
}
...
static void ice_prune_candidate_pairs(
    IceCheckList *cl)
{
    MSList *list;
    MSList *next;
    ...
    {
        next = list->next;
        if (ice_prune_duplicate_pair((IceCandidatePair *)list->data, &cl->pairs, cl))
        {
    ...
    if (nb_pairs > cl->session->max_connectivity_checks)
    {
        ...
        for (i = 0; i < nb_pairs_to_remove; i++)
        {
            ...
            ice_free_candidate_pair((IceCandidatePair *)list->data, cl);
            prev           = list->prev;
...
static int ice_find_pair_foundation(
    const IcePairFoundation *f1, const IcePairFoundation *f2)
{
...
static void ice_generate_pair_foundations_list(
    const IceCandidatePair *pair, MSList **list)
{
    IcePairFoundation foundation;
...
static void ice_find_lowest_componentid_pair_with_specified_foundation(
    IceCandidatePair *pair, Foundation_Pair_Priority_ComponentID *fc)
{
...
static void ice_set_lowest_componentid_pair_with_foundation_to_waiting_state(
    const IcePairFoundation *foundation, IceCheckList *cl)
{
...
static void ice_session_pair_candidates(
    IceSession *session)
{
    MSList       *elem;
    IceCheckList *cl;
...
void ice_session_start_connectivity_checks(
    IceSession *session)
{
...
static void ice_perform_regular_nomination(
    IceValidCandidatePair *valid_pair, CheckList_RtpSession *cr)
{
...
static void ice_remove_waiting_and_frozen_pairs_from_list(
    MSList **list, uint16_t componentID)
{
    IceCandidatePair *pair;
    MSList           *elem;
...
static void ice_stop_retransmission_for_in_progress_pair(
    IceCandidatePair *pair, const uint16_t *componentID)
{
...
static void ice_conclude_waiting_frozen_and_inprogress_pairs(
    const IceValidCandidatePair *valid_pair, IceCheckList *cl)
{
...
static int ice_find_use_candidate_valid_pair_from_componentID(
    const IceValidCandidatePair *valid_pair, const uint16_t *componentID)
{
    return !((valid_pair->generated_from->use_candidate == TRUE) && (valid_pair->generated_from->local->componentID == *componentID));
}

static int ice_find_nominated_valid_pair_from_componentID(
    const IceValidCandidatePair *valid_pair, const uint16_t *componentID)
{
    return !((valid_pair->valid->is_nominated == TRUE) && (valid_pair->valid->local->componentID == *componentID));
}

static void ice_find_nominated_valid_pair_for_componentID(
    const uint16_t *componentID, CheckList_Bool *cb)
{
    MSList *elem = ms_list_find_custom(cb->cl->valid_list, (MSCompareFunc)ice_find_nominated_valid_pair_from_componentID, (void *)componentID);
    if (elem == NULL)
    {
...
static int ice_find_selected_valid_pair_from_componentID(
    const IceValidCandidatePair *valid_pair, const uint16_t *componentID)
{
    return !((valid_pair->selected == TRUE) && (valid_pair->valid->local->componentID == *componentID));
}

static void ice_find_selected_valid_pair_for_componentID(
    const uint16_t *componentID, CheckList_Bool *cb)
{
    MSList *elem = ms_list_find_custom(cb->cl->valid_list, (MSCompareFunc)ice_find_selected_valid_pair_from_componentID, (void *)componentID);
    if (elem == NULL)
    {
        /* This component ID is not present in the valid list. */
        cb->result = FALSE;
...
static void ice_check_all_pairs_in_failed_or_succeeded_state(
    const IceCandidatePair *pair, CheckList_Bool *cb)
{
...
static void ice_pair_stop_retransmissions(
    IceCandidatePair *pair, IceCheckList *cl)
{
    MSList *elem;
    if (pair->state == ICP_InProgress)
...
static void ice_check_list_stop_retransmissions(
    IceCheckList *cl)
{
    ms_list_for_each2(cl->check_list, (void (*)(void *, void *))ice_pair_stop_retransmissions, cl);
}

static int ice_find_running_check_list(
    const IceCheckList *cl)
{
    return !(cl->state == ICL_Running);
}

static int ice_find_unsuccessful_check_list(
    IceCheckList *cl, const void *dummy)
{
    return (cl->state == ICL_Completed);
}

static void ice_continue_processing_on_next_check_list(
    IceCheckList *cl, RtpSession *rtp_session)
{
...
static void ice_conclude_processing(
    IceCheckList *cl, RtpSession *rtp_session)
{
    CheckList_RtpSession cr;
    CheckList_Bool       cb;
...
static void ice_check_list_restart(
    IceCheckList *cl)
{
    if (cl->remote_ufrag) ms_free(cl->remote_ufrag);
    if (cl->remote_pwd) ms_free(cl->remote_pwd);
...
static void ice_check_gathering_timeout_of_check_list(
    const IceCheckList *cl, Time_Bool *tb)
{
...
static void ice_check_list_stop_gathering(
    IceCheckList *cl)
{
    cl->gathering_candidates = FALSE;
    cl->gathering_finished   = TRUE;
...
static bool_t ice_check_gathering_timeout(
    IceCheckList *cl, RtpSession *rtp_session, MSTimeSpec curtime)
{
    Time_Bool tb;
    OrtpEvent *ev;
...
static void ice_send_stun_server_checks(
    IceStunServerCheck *check, IceCheckList *cl)
{
    MSTimeSpec curtime = ice_current_time();
...
static void ice_handle_connectivity_check_retransmission(
    IceCandidatePair *pair, const CheckList_RtpSession_Time *params)
{
...
static int ice_find_pair_from_state(
    const IceCandidatePair *pair, const IceCandidatePairState *state)
{
    return !(pair->state == *state);
}

static void ice_check_retransmissions_pending(
    const IceCandidatePair *pair, bool_t *retransmissions_pending)
{
    if ((pair->state == ICP_InProgress) && (pair->retransmissions <= ICE_MAX_RETRANSMISSIONS))
        *retransmissions_pending = TRUE;
}

static void ice_check_list_retransmit_connectivity_checks(
    IceCheckList *cl, RtpSession *rtp_session, MSTimeSpec curtime)
{
...
static IceCandidatePair *ice_check_list_send_triggered_check(
    IceCheckList *cl, RtpSession *rtp_session)
{
...
static MSTimeSpec ice_current_time(
    void)
{
    MSTimeSpec cur_time;
...
static MSTimeSpec ice_add_ms(
    MSTimeSpec orig, uint32_t ms)
{
    if (ms == 0) return orig;
    orig.tv_sec  += ms / 1000;
...
static int32_t ice_compare_time(
    MSTimeSpec ts1, MSTimeSpec ts2)
{
...
static char *ice_inet_ntoa(
    struct sockaddr *addr, int addrlen, char *dest, int destlen)
{
    int err;
    dest[0] = '\0';
...
static void transactionID2string(
    const UInt96 *tr_id, char *tr_id_str)
{
    int j, pos;
...
static void ice_set_credentials(
    char **ufrag, char **pwd, const char *ufrag_str, const char *pwd_str)
{
    ...
    if (*pwd) ms_free(*pwd);
    *ufrag              = (char *)ms_malloc(len_ufrag + 1);
    strncpy(*ufrag, ufrag_str, len_ufrag);
    (*ufrag)[len_ufrag] = '\0';
    *pwd                = (char *)ms_malloc(len_pwd + 1);
    strncpy(*pwd,   pwd_str,   len_pwd);
    (*pwd)[len_pwd]     = '\0';
}

static int ice_find_host_candidate(
    const IceCandidate *candidate, const uint16_t *componentID)
{
...
static void ice_set_base_for_srflx_candidate(
    IceCandidate *candidate, IceCandidate *base)
{
    if ((candidate->type == ICT_ServerReflexiveCandidate) && (candidate->base == NULL) && (candidate->componentID == base->componentID))
        candidate->base = base;
}

static void ice_set_base_for_srflx_candidate_with_componentID(
    uint16_t *componentID, IceCheckList *cl)
{
    IceCandidate *base;
...
static void ice_get_valid_pair_for_componentID(
    const uint16_t *componentID, CheckList_MSListPtr *cm)
{
    MSList *elem = ms_list_find_custom(cm->cl->valid_list, (MSCompareFunc)ice_find_nominated_valid_pair_from_componentID, (void *)componentID);
    if (elem != NULL)
    {
        IceValidCandidatePair *valid_pair = (IceValidCandidatePair *)elem->data;
...
static MSList *ice_get_valid_pairs(
    const IceCheckList *cl)
{
...
static void ice_get_remote_addr_and_ports_from_valid_pair(
    const IceCandidatePair *pair, Addr_Ports *addr_ports)
{
    if (pair->local->componentID == 1)
    {
...
void ice_get_remote_addr_and_ports_from_valid_pairs(
...
static void ice_get_local_addr_and_ports_from_valid_pair(
    const IceCandidatePair *pair, Addr_Ports *addr_ports)
{
    if (pair->local->componentID == 1)
    {
...
static void ice_get_local_addr_and_ports_from_valid_pairs(
    const IceCheckList *cl, char *rtp_addr, int *rtp_port, char *rtcp_addr, int *rtcp_port, int addr_len)
{
    Addr_Ports addr_ports;
    MSList     *ice_pairs = ice_get_valid_pairs(cl);
...
void ice_check_list_print_route(
    const IceCheckList *cl, const char *message)
{
...
static void ice_dump_candidate(
    const IceCandidate *candidate, const char *const prefix)
{
...
static void ice_dump_candidate_pair(
    const IceCandidatePair *pair, int *i)
{
...
static void ice_dump_candidate_pair_foundation(
    const IcePairFoundation *foundation)
{
    ms_message("\t%s\t%s", foundation->local, foundation->remote);
}

void ice_dump_candidate_pairs_foundations(
    const IceCheckList *cl)
{
    if (cl == NULL) return;
...
static void ice_dump_valid_pair(
    const IceValidCandidatePair *valid_pair, int *i)
{
    int j = *i;
...
void ice_dump_triggered_checks_queue(
    const IceCheckList *cl)
{
    int i = 1;
    if (cl == NULL) return;
...
static void ice_dump_componentID(
    const uint16_t *componentID)
{
    ms_message("\t%u", *componentID);
}

void ice_dump_componentIDs(
    const IceCheckList *cl)
{
    if (cl == NULL) return;
