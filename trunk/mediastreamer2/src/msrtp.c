...
struct SenderData {
    RtpSession *session;
    uint32_t   tsoff;
    uint32_t   last_ts; // last time stamp
    int64_t    last_sent_time;
    int64_t    last_stun_sent_time;
    uint32_t   skip_until;
    int        rate;      // sampling rate
    int        nchannels; // channel number
    int        dtmf_duration;
    int        dtmf_ts_step;
...
static int sender_set_dtmf_duration(
    MSFilter *f, void *arg)
{
    SenderData *d = (SenderData *) f->data;
...
static int sender_set_relay_session_id(
    MSFilter *f, void *arg)
{
    SenderData *d   = (SenderData *) f->data;
...
static uint32_t get_cur_timestamp(
    MSFilter *f, mblk_t *im)
{
    SenderData *d    = (SenderData *)f->data;
...
static int send_dtmf(
    MSFilter *f, uint32_t timestamp_start)
{
    SenderData *d = (SenderData *) f->data;
    mblk_t     *m1;
    int        tev_type;
    ...
    MSFilter *f)
{
    ...
    if (f != NULL && f->data != NULL)
    {
        ...
        ms_filter_lock(f);
        im = ms_queue_get(f->inputs[0]);    // get data from input
        do
        {
            mblk_t *header;

            timestamp = get_cur_timestamp(f, im);   // get current timestamp
            ...
            if (im)
            {
                ...
                {
                    header         = rtp_session_create_packet(s, 12, NULL, 0); // create rtp packet
...
#if 0
    ms_warning("%s: time %llu, dur %llu\n", f->desc->name, GetTickCount64() - start, GetTickCount64() - ((SenderData *)f->data)->pre);
    ((SenderData *)f->data)-> pre = GetTickCount64();
#endif
}

static void sender_process(
    MSFilter *f)
{
    if (f != NULL && f->data != NULL)
    {
        ...
        if (s == NULL)
        {
            ms_queue_flush(f->inputs[0]);   // flush input data
            return;
        }
        if (d->use_task)
...
struct ReceiverData {
    ...
    int        rate;        // sampling rate
    int        nchannels;   // channel number
    bool_t     starting;    // used to indicate if it is in starting state
    bool_t     reset_jb;
};
...
static void receiver_postprocess(
    MSFilter *f)
{
    /*ReceiverData *d = (ReceiverData *) f->data;*/
}

static void receiver_uninit(
    MSFilter *f)
{
    if (f != NULL && f->data != NULL)
    {
...
static int receiver_set_session(
    MSFilter *f, void *arg)
{
    ReceiverData *d  = (ReceiverData *) f->data;
    RtpSession   *s  = (RtpSession *) arg;
...
static int receiver_reset_jitter_buffer(
    MSFilter *f, void *arg)
{
    ReceiverData *d = (ReceiverData *)f->data;
    d->reset_jb = TRUE;
...
static void receiver_preprocess(
    MSFilter *f)
{
    if (f != NULL && f->data != NULL)
    {
