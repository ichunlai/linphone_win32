...
MSTicker *ms_ticker_new_with_params(
...
void ms_ticker_set_priority(
    MSTicker *ticker, MSTickerPrio prio)
{
    ticker->prio = prio;
}

static void ms_ticker_uninit(
    MSTicker *ticker)
{
...
static bool_t filter_can_process(
    MSFilter *f, int tick)
{
...
static void run_graph(
    MSFilter *f, MSTicker *s, MSList **unschedulable, bool_t force_schedule)
{
    int     i;
    MSQueue *l;
...
static void run_graphs(
    MSTicker *s, MSList *execution_list, bool_t force_schedule)
{
    MSList *it;
...
static void remove_tasks_for_filter(
    MSTicker *ticker, MSFilter *f)
{
...
static int wait_next_tick(
    void *data, uint64_t virt_ticker_time)
{
...
void ms_ticker_set_time_func(
    MSTicker *ticker, MSTickerTimeFunc func, void *user_data)
{
...
void ms_ticker_set_tick_func(
    MSTicker *ticker, MSTickerTickFunc func, void *user_data)
{
    if (func == NULL)
    {
        func      = wait_next_tick;
...
static uint64_t get_wallclock_ms(
    void)
{
    MSTimeSpec ts;
    ms_get_cur_time(&ts);
