...
void ms_web_cam_manager_destroy(
    void)
{
    if (scm != NULL)
    {
...
MSWebCamManager *ms_web_cam_manager_get(
    void)
{
...
MSWebCam *ms_web_cam_manager_get_cam(
    MSWebCamManager *m, const char *id)
{
    MSList *elem;
...
MSWebCam *ms_web_cam_manager_get_default_cam(
    MSWebCamManager *m)
{
    if (m->cams != NULL)
...
const MSList *ms_web_cam_manager_get_list(
    MSWebCamManager *m)
{
    return m->cams;
}

void ms_web_cam_manager_add_cam(
    MSWebCamManager *m, MSWebCam *c)
{
    ms_message("Webcam %s added", ms_web_cam_get_string_id(c));
    m->cams = ms_list_append(m->cams, c);
}

void ms_web_cam_manager_prepend_cam(
    MSWebCamManager *m, MSWebCam *c)
{
...
static void cam_detect(
    MSWebCamManager *m, MSWebCamDesc *desc)
{
    if (desc->detect != NULL)
        desc->detect(m);
}

void ms_web_cam_manager_register_desc(
    MSWebCamManager *m, MSWebCamDesc *desc)
{
...
void ms_web_cam_manager_reload(
    MSWebCamManager *m)
{
    MSList *elem;
...
const char *ms_web_cam_get_driver_type(
    const MSWebCam *obj)
{
    return obj->desc->driver_type;
}

const char *ms_web_cam_get_name(
    const MSWebCam *obj)
{
    return obj->name;
}

const char *ms_web_cam_get_string_id(
    MSWebCam *obj)
{
...
struct _MSFilter *ms_web_cam_create_reader(
    MSWebCam *obj)
{
