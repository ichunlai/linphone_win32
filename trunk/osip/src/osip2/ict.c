...
int __osip_ict_init(
    osip_ict_t **ict, osip_t *osip, osip_message_t *invite)
{
    osip_route_t *route;
    int          i;
    time_t       now;
    ...
    *ict = (osip_ict_t *) osip_malloc(sizeof(osip_ict_t));
    ...
    else
    {
        ...
        if (maddr_param != NULL && maddr_param->gvalue != NULL)
            osip_ict_set_destination((*ict), osip_strdup(maddr_param->gvalue),
                                     port);
        else
            osip_ict_set_destination((*ict), osip_strdup(invite->req_uri->host),
                                     port);
    }
...
int osip_ict_set_destination(
    osip_ict_t *ict, char *destination, int port)
{
    if (ict == NULL)
...
osip_event_t *__osip_ict_need_timer_a_event(
    osip_ict_t *ict, state_t state,
    int transactionid)
{
...
osip_event_t *__osip_ict_need_timer_b_event(
    osip_ict_t *ict, state_t state,
    int transactionid)
{
