...
#ifndef MINISIZE
    __ict_load_fsm();   // init ICT (INVITE client transaction) state machine
    __ist_load_fsm();   // init IST (INVITE server transaction) state machine
    __nict_load_fsm();  // init NICT (NON-INVITE client transaction) state machine
    __nist_load_fsm();  // init NIST (NON-INVITE server transaction) state machine
#endif
    ...
    struct timeval *current)
{
    if (osip_timercmp(current, &ixt->start, >))
    {
        ...
        if (ixt->ack != NULL)
            ...
            osip->cb_send_message(NULL, ixt->msg2xx, ixt->dest, ixt->port,
                                  ixt->sock);
        ixt->counter--;
    }
}
...
#endif
    /* handle ict timers */
    tr = (osip_transaction_t *) osip_list_get_first(&osip->osip_ict_transactions,
                                                   &iterator);
    while (osip_list_iterator_has_elem(iterator))
    {
...
#endif
    /* handle ist timers */
    tr = (osip_transaction_t *) osip_list_get_first(&osip->osip_ist_transactions,
                                                   &iterator);
    while (osip_list_iterator_has_elem(iterator))
    {
...
#endif
    /* handle nict timers */
    tr = (osip_transaction_t *) osip_list_get_first(&osip->osip_nict_transactions,
                                                   &iterator);
    while (osip_list_iterator_has_elem(iterator))
    {
...
#endif
    /* handle nist timers */
    tr = (osip_transaction_t *) osip_list_get_first(&osip->osip_nist_transactions,
                                                   &iterator);
    while (osip_list_iterator_has_elem(iterator))
    {
