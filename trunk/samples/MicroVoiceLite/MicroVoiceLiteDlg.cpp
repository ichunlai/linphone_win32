...
#include "private.h"
#include "../../linphone/console/linphonec.h"
...
static void
linphone_log_handler(OrtpLogLevel lev, const char *fmt, va_list args)
{
...
#ifdef _MSC_VER
    CString cstmsg(msg_str);
    OutputDebugString(cstmsg);
#endif
...
static void
showInterfaceCb(LinphoneCore *lc)
{
...
static void
inviteReceivedCb(LinphoneCore *lc, const char *from)
{
    CMicroVoiceLiteDlg *the_ui = (CMicroVoiceLiteDlg *)linphone_core_get_user_data(lc);
    ...
    if (the_ui)
    {
        tmp.Format(_T("新来电 (%s)"), from);

        if (IDYES == AfxMessageBox(tmp + _T("\n \t\t\t是否接听?"), MB_YESNO | MB_DEFBUTTON2))
...
static void
byeReceivedCb(LinphoneCore *lc, const char *from) {}

static void
displayStatusCb(LinphoneCore *lc, const char *message)
{
    CMicroVoiceLiteDlg *the_ui = (CMicroVoiceLiteDlg *)linphone_core_get_user_data(lc);
#ifdef _MSC_VER
    CString cstmsg(message);
    OutputDebugString(cstmsg);
#endif
    if (the_ui)
...
static void
displayMessageCb(LinphoneCore *lc, const char *message) {}

static void
authInfoRequested(LinphoneCore *lc, const char *realm, const char *username) {}

static void
network_quality(LinphoneCore *lc, int ms)
{
    CMicroVoiceLiteDlg *the_ui = (CMicroVoiceLiteDlg *)linphone_core_get_user_data(lc);
    CString            tmp;

    tmp.Format(_T("网络延时 %d ms"), ms);
    if (the_ui)
        the_ui->mStatus.SetWindowText(tmp);
}

static void
media_quality(LinphoneCore *lc, int jitter, float packet_lost)
{
    CMicroVoiceLiteDlg *the_ui = (CMicroVoiceLiteDlg *)linphone_core_get_user_data(lc);
    CString            tmp;

    tmp.Format(_T("包间平均抖动 %d ms, 丢包率 %2.2f%%"), jitter / 20, packet_lost);
    if (the_ui)
        the_ui->mStatus.SetWindowText(tmp);
}
...
void
CMicroVoiceLiteDlg::DoDataExchange(CDataExchange *pDX)
{
    CDialog::DoDataExchange(pDX);
...
void
CMicroVoiceLiteDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR
CMicroVoiceLiteDlg::OnQueryDragIcon()
{
    return static_cast<HCURSOR>(m_hIcon);
}

void
CMicroVoiceLiteDlg::OnDestroy()
{
    CDialog::OnDestroy();
...
void
CMicroVoiceLiteDlg::OnTimer(UINT_PTR nIDEvent)
{
...
CMicroVoiceLiteDlg::OnBnClickedBntMakecall()
{
    ...
    char buf[512];
    LinphoneCall *call;
    LinphoneCallParams* cp = linphone_core_create_default_call_parameters(the_core);
    const char      *uri = linphone_core_get_identity(the_core);
    LinphoneAddress *tmpAddress = linphone_address_new(uri);
    ...
    linphonec_parse_command_line(the_core, buf);
    snprintf(buf, sizeof(buf), "firewall ice");
#else
    snprintf(buf, sizeof(buf), "firewall none");
#endif
    linphonec_parse_command_line(the_core, buf);

    linphone_call_params_enable_early_media_sending(cp, TRUE);
    if (NULL == (call = linphone_core_invite_address_with_params(the_core, tmpAddress, cp)))
    {
        ms_warning("Error from linphone_core_invite.\n");
    }
end:
    if (cp) linphone_call_params_destroy(cp);
    if (tmpAddress) linphone_address_destroy(tmpAddress);
#endif
}

// SIP REGISTER
void
CMicroVoiceLiteDlg::OnBnClickedBntRegister()
{
...
void
CMicroVoiceLiteDlg::OnBnClickedBntDropcall()
{
    linphone_core_terminate_call(the_core, NULL);
}

void
CMicroVoiceLiteDlg::OnBnClickedBntUnregister()
{
    /*LinphoneProxyConfig* proxyCfg=NULL;
       //get default proxy
       linphone_core_get_default_proxy(the_core,&proxyCfg);
...
void
CMicroVoiceLiteDlg::OnBnClickedCancel()
{
    // TODO: 在此添加控件通知处理程序代码
    OnCancel();
}
...
void
CMicroVoiceLiteDlg::OnBnClickedCheckHold()
{
}

void
CMicroVoiceLiteDlg::OnBnClickedCheckAgc()
{
    if (mAGC.GetCheck() == 1)
    {
...
void
CMicroVoiceLiteDlg::OnBnClickedCheckNr()
{
}

void
CMicroVoiceLiteDlg::OnBnClickedCheckRecord()
{
}

void
CMicroVoiceLiteDlg::OnBnClickedCheckAec()
{
    if (mAEC.GetCheck() == 1)
    {
...
void
CMicroVoiceLiteDlg::OnBnClickedBntSendDtmf()
{
    CString    dtmf;
...
void
CMicroVoiceLiteDlg::OnBnClickedButtonDtmf1()
{
    linphone_core_send_dtmf(the_core, '1');
}

void
CMicroVoiceLiteDlg::OnBnClickedButtonDtmf2()
{
    linphone_core_send_dtmf(the_core, '2');
}

void
CMicroVoiceLiteDlg::OnBnClickedButtonDtmf3()
{
    linphone_core_send_dtmf(the_core, '3');
}

void
CMicroVoiceLiteDlg::OnBnClickedButtonDtmf4()
{
    linphone_core_send_dtmf(the_core, '4');
}

void
CMicroVoiceLiteDlg::OnBnClickedButtonDtmf5()
{
    linphone_core_send_dtmf(the_core, '5');
}

void
CMicroVoiceLiteDlg::OnBnClickedButtonDtmf6()
{
    linphone_core_send_dtmf(the_core, '6');
}

void
CMicroVoiceLiteDlg::OnBnClickedButtonDtmf7()
{
    linphone_core_send_dtmf(the_core, '7');
}

void
CMicroVoiceLiteDlg::OnBnClickedButtonDtmf8()
{
    linphone_core_send_dtmf(the_core, '8');
}

void
CMicroVoiceLiteDlg::OnBnClickedButtonDtmf9()
{
    linphone_core_send_dtmf(the_core, '9');
}

void
CMicroVoiceLiteDlg::OnBnClickedButtonDtmf10()
{
    linphone_core_send_dtmf(the_core, '*');
}

void
CMicroVoiceLiteDlg::OnBnClickedButtonDtmf0()
{
    linphone_core_send_dtmf(the_core, '0');
}

void
CMicroVoiceLiteDlg::OnBnClickedButtonDtmf11()
{
    linphone_core_send_dtmf(the_core, '#');
}

void
CMicroVoiceLiteDlg::OnCbnSelchangeComboCamList()
{
    CString device;
