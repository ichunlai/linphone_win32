...
void mblk_meta_copy(
    const mblk_t *source, mblk_t *dest)
{
...
static inline void datab_ref(
    dblk_t *d)
{
    d->db_ref++;
}

static inline void datab_unref(
    dblk_t *d)
{
    d->db_ref--;
    if (d->db_ref == 0)
...
mblk_t *esballoc(
    uint8_t *buf, int size, int pri, void (*freefn)(void *))
{
    mblk_t *mp;
    dblk_t *datab;
...
mblk_t *appendb(
    mblk_t *mp, const char *data, int size, bool_t pad)
{
    int padcnt = 0;
    int i;
    if (pad)
...
void msgb_allocator_init(
    msgb_allocator_t *a)
{
    qinit(&a->q);
}

mblk_t *msgb_allocator_alloc(
    msgb_allocator_t *a, int size)
{
    queue_t *q = &a->q;
    mblk_t  *m, *found = NULL;
