...
static void payload_type_changed(
    RtpSession *session, PayloadType *pt)
{
...
bool_t wait_point_check(
    WaitPoint *wp, uint32_t t)
{
    bool_t ok = FALSE;
...
mblk_t *rtp_getq(
    queue_t *q, uint32_t timestamp, int *rejected)
{
...
void rtp_session_set_rtcp_report_interval(
    RtpSession *session, int value_ms)
{
...
void rtp_session_set_recv_buf_size(
    RtpSession *session, int bufsize)
{
...
rtp_session_signal_connect(
    RtpSession *session, const char *signal_name,
 ...
 **/
uint16_t rtp_session_get_seq_number(
    RtpSession *session)
{
 ...
 **/
uint32_t rtp_session_get_recv_ssrc(
    RtpSession *session)
{
    return session->rcv.ssrc;
}

void rtp_session_update_payload_type(
...
rtp_session_send_with_ts(
    RtpSession *session, const uint8_t *buffer, int len,
    uint32_t userts)
{
    mblk_t *m;
    int    err;
...
static void payload_type_changed_notify(
    RtpSession *session, int paytype)
{
    PayloadType *pt = rtp_profile_get_payload(session->rcv.profile, paytype);
    if (pt)
...
rtp_session_pick_with_cseq(
    RtpSession *session, const uint16_t sequence_number)
{
 ...
 **/
uint32_t rtp_session_get_current_send_ts(
    RtpSession *session)
{
    uint32_t     userts;
 ...
 *
 **/
uint32_t rtp_session_get_current_recv_ts(
    RtpSession *session)
{
    uint32_t     userts;
    uint32_t     session_time;
...
void rtp_session_set_time_jump_limit(
    RtpSession *session, int milisecs)
{
    uint32_t ts;
 ...
 **/
void rtp_session_register_event_queue(
    RtpSession *session, OrtpEvQueue *q)
{
    session->eventqs = o_list_append(session->eventqs, q);
}

void rtp_session_unregister_event_queue(
    RtpSession *session, OrtpEvQueue *q)
{
    session->eventqs = o_list_remove(session->eventqs, q);
}

void rtp_session_dispatch_event(
    RtpSession *session, OrtpEvent *ev)
{
    OList *it;
    int   i;
...
void rtp_session_set_ssrc_changed_threshold(
    RtpSession *session, int numpackets)
{
 ...
 **/
const rtp_stats_t *rtp_session_get_stats(
    const RtpSession *session)
{
 ...
 **/
const jitter_stats_t *rtp_session_get_jitter_stats(
    const RtpSession *session)
{
...
static float compute_bw(
    struct timeval *orig, unsigned int bytes)
{
 ...
 **/
int rtp_get_payload(
    mblk_t *packet, unsigned char **start)
{
    unsigned char *tmp;
...
rtp_session_get_last_recv_time(
    RtpSession *session, struct timeval *tv)
{
#ifdef PERF
...
uint32_t rtp_session_time_to_ts(
    RtpSession *session, int millisecs)
{
    PayloadType *payload;
    payload =
...
uint32_t rtp_session_ts_to_time(
...
void rtp_session_process(
    RtpSession *session, uint32_t time, RtpScheduler *sched)
{
    wait_point_lock(&session->snd.wp);
