...
static bool_t try_connect(
    int fd, const struct sockaddr *dest, socklen_t addrlen)
{
...
static ortp_socket_t create_and_bind(
    const char *addr, int port, int *sock_family, bool_t reuse_addr)
{
    int                err;
    int                optval = 1;
...
static void set_socket_sizes(
    int sock, unsigned int sndbufsz, unsigned int rcvbufsz)
{
    int    err;
    bool_t done = FALSE;
    if (sndbufsz > 0)
...
static ortp_socket_t create_and_bind_random(
    const char *localip, int *sock_family, int *port)
{
    int           retry;
    ortp_socket_t sock = -1;
...
int
rtp_session_set_local_addr(
    RtpSession *session, const char *addr, int rtp_port, int rtcp_port)
{
    ortp_socket_t sock;
    int           sockfamily;
...
int rtp_session_set_pktinfo(
    RtpSession *session, int activate)
{
    int  retval;
    int  optname;
#if defined(WIN32) || defined(_WIN32_WCE)
...
int
rtp_session_set_remote_addr(
    RtpSession *session, const char *addr, int port)
{
...
void rtp_session_set_sockets(
    RtpSession *session, int rtpfd, int rtcpfd)
{
...
void rtp_session_set_transports(
    RtpSession *session, struct _RtpTransport *rtptr, struct _RtpTransport *rtcptr)
{
...
#ifdef USE_SENDMSG
    #define MAX_IOV 30
static int rtp_sendmsg(
    int sock, mblk_t *m, struct sockaddr *rem_addr, int addr_len)
{
    int           error;
    struct msghdr msg;
...
static void update_sent_bytes(
    RtpSession *s, int nbytes)
{
#ifdef ORTP_INET6
...
void rtp_session_notify_inc_rtcp(
    RtpSession *session, mblk_t *m)
{
...
static void compute_rtt(
    RtpSession *session, const struct timeval *now, const report_block_t *rb)
{
 ...
 */
static int process_rtcp_packet(
    RtpSession *session, mblk_t *block, struct sockaddr *addr, socklen_t addrlen)
{
