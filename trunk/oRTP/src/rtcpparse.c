...
const rtcp_common_header_t *rtcp_get_common_header(
    const mblk_t *m)
{
    int                  size = msgdsize(m);
...
const sender_info_t *rtcp_SR_get_sender_info(
    const mblk_t *m)
{
    rtcp_sr_t *sr = (rtcp_sr_t *)m->b_rptr;
    return &sr->si;
...
const report_block_t *rtcp_SR_get_report_block(
    const mblk_t *m, int idx)
{
    rtcp_sr_t      *sr  = (rtcp_sr_t *)m->b_rptr;
...
const report_block_t *rtcp_RR_get_report_block(
    const mblk_t *m, int idx)
{
    rtcp_rr_t      *rr  = (rtcp_rr_t *)m->b_rptr;
...
void rtcp_sdes_parse(
    const mblk_t *m, SdesItemFoundCallback cb, void *user_data)
{
...
bool_t rtcp_BYE_get_ssrc(
    const mblk_t *m, int idx, uint32_t *ssrc)
{
...
bool_t rtcp_BYE_get_reason(
    const mblk_t *m, const char **reason, int *reason_len)
{
    rtcp_bye_t *bye  = (rtcp_bye_t *)m->b_rptr;
    int        rc    = rtcp_common_header_get_rc(&bye->ch);
...
void rtcp_APP_get_data(
    const mblk_t *m, uint8_t **data, int *len)
{
