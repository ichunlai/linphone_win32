...
SalTransport sal_transport_parse(
    const char *param)
{
...
static void sal_media_description_destroy(
...
void sal_media_description_ref(
    SalMediaDescription *md)
{
    md->refcount++;
}

void sal_media_description_unref(
    SalMediaDescription *md)
{
    md->refcount--;
    if (md->refcount == 0)
...
SalStreamDescription *sal_media_description_find_stream(
    SalMediaDescription *md,
...
bool_t sal_media_description_empty(
...
void sal_media_description_set_dir(
    SalMediaDescription *md, SalStreamDir stream_dir)
{
    int i;
    for (i = 0; i < md->n_active_streams; ++i)
    {
...
static bool_t has_dir(
    const SalMediaDescription *md, SalStreamDir stream_dir)
{
    int i;
...
bool_t sal_media_description_has_dir(
    const SalMediaDescription *md, SalStreamDir stream_dir)
{
...
static bool_t payload_type_equals(
    const PayloadType *p1, const PayloadType *p2)
{
...
static bool_t payload_list_equals(
    const MSList *l1, const MSList *l2)
{
    const MSList *e1, *e2;
...
int sal_stream_description_equals(
    const SalStreamDescription *sd1, const SalStreamDescription *sd2)
{
...
int sal_media_description_equals(
    const SalMediaDescription *md1, const SalMediaDescription *md2)
{
    int result = SAL_MEDIA_DESCRIPTION_UNCHANGED;
    int i;
...
void sal_op_set_contact(
    SalOp *op, const char *contact)
{
    assign_string(&((SalOpBase *)op)->contact, contact);
}

void sal_op_set_route(
    SalOp *op, const char *route)
{
    assign_string(&((SalOpBase *)op)->route, route);
}

void sal_op_set_from(
    SalOp *op, const char *from)
{
    assign_string(&((SalOpBase *)op)->from, from);
}

void sal_op_set_to(
    SalOp *op, const char *to)
{
    assign_string(&((SalOpBase *)op)->to, to);
}

void sal_op_set_user_pointer(
    SalOp *op, void *up)
{
...
const char *sal_op_get_from(
    const SalOp *op)
{
    return ((SalOpBase *)op)->from;
}

const char *sal_op_get_to(
    const SalOp *op)
{
    return ((SalOpBase *)op)->to;
}

const char *sal_op_get_contact(
    const SalOp *op)
{
    return ((SalOpBase *)op)->contact;
}

const char *sal_op_get_remote_contact(
    const SalOp *op)
{
    return ((SalOpBase *)op)->remote_contact;
}

const char *sal_op_get_route(
    const SalOp *op)
{
    return ((SalOpBase *)op)->route;
}

const char *sal_op_get_remote_ua(
    const SalOp *op)
{
    return ((SalOpBase *)op)->remote_ua;
}

void *sal_op_get_user_pointer(
    const SalOp *op)
{
    return ((SalOpBase *)op)->user_pointer;
}

const char *sal_op_get_proxy(
    const SalOp *op)
{
    return ((SalOpBase *)op)->route;
}

const char *sal_op_get_network_origin(
    const SalOp *op)
{
    return ((SalOpBase *)op)->origin;
}

const char *sal_op_get_call_id(
    const SalOp *op)
{
...
void __sal_op_set_network_origin(
    SalOp *op, const char *origin)
{
    assign_string(&((SalOpBase *)op)->origin, origin);
}

void __sal_op_set_remote_contact(
    SalOp *op, const char *ct)
{
...
SalAuthInfo *sal_auth_info_clone(
...
SalCustomHeader *sal_custom_header_append(
    SalCustomHeader *ch, const char *name, const char *value)
{
...
const char *sal_custom_header_find(
    const SalCustomHeader *ch, const char *name)
{
    const MSList *it;
...
SalCustomHeader *sal_custom_header_clone(
...
const SalCustomHeader *sal_op_get_custom_header(
    SalOp *op)
{
    SalOpBase *b = (SalOpBase *)op;
 ...
 */
void sal_op_set_custom_header(
    SalOp *op, SalCustomHeader *ch)
{
    SalOpBase *b = (SalOpBase *)op;
    if (b->custom_headers)
    {
