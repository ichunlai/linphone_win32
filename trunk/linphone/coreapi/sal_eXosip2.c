...
void _osip_list_set_empty(
    osip_list_t *l, void (*freefunc)(void *))
{
    void *data;
...
void sal_get_default_local_ip(
    Sal *sal, int address_family, char *ip, size_t iplen)
{
...
static void sal_add_call(
    Sal *sal, SalOp *op)
{
    sal->calls = ms_list_append(sal->calls, op);
}

static void sal_remove_call(
    Sal *sal, SalOp *op)
{
    sal->calls = ms_list_remove(sal->calls, op);
}

static SalOp *sal_find_register(
    Sal *sal, int rid)
{
    const MSList *elem;
    SalOp        *op;
...
static void sal_add_register(
    Sal *sal, SalOp *op)
{
    sal->registers = ms_list_append(sal->registers, op);
}

static void sal_remove_register(
    Sal *sal, int rid)
{
    MSList *elem;
    SalOp  *op;
...
static SalOp *sal_find_other(
    Sal *sal, osip_message_t *message)
{
    const MSList   *elem;
    SalOp          *op;
...
void sal_add_other(
    Sal *sal, SalOp *op, osip_message_t *request)
{
...
bool_t sal_call_autoanswer_asked(
    SalOp *op)
{
...
static void _osip_trace_func(
    char *fi, int li, osip_trace_level_t level, char *chfr, va_list ap)
{
...
void sal_set_user_pointer(
    Sal *sal, void *user_data)
{
    sal->up = user_data;
}

void *sal_get_user_pointer(
    const Sal *sal)
{
    return sal->up;
}
...
void sal_set_callbacks(
    Sal *ctx, const SalCallbacks *cbs)
{
...
int sal_listen_port(
    Sal *ctx, const char *addr, int port, SalTransport tr, int is_secure)
{
    int    err;
    bool_t ipv6;
...
void sal_set_user_agent(
    Sal *ctx, const char *user_agent)
{
    eXosip_set_user_agent(user_agent);
}

void sal_use_session_timers(
    Sal *ctx, int expires)
{
    ctx->session_expires = expires;
}

void sal_use_one_matching_codec_policy(
    Sal *ctx, bool_t one_matching_codec)
{
    ctx->one_matching_codec = one_matching_codec;
}

MSList *sal_get_pending_auths(
    Sal *sal)
{
    return ms_list_copy(sal->pending_auths);
}

void sal_use_double_registrations(
...
void sal_use_rport(
    Sal *ctx, bool_t use_rports)
{
    ctx->use_rports = use_rports;
}

void sal_use_101(
    Sal *ctx, bool_t use_101)
{
    ctx->use_101 = use_101;
}

void sal_set_root_ca(
    Sal *ctx, const char *rootCa)
{
    if (ctx->rootCa)
        ms_free(ctx->rootCa);
...
const char *sal_get_root_ca(
    Sal *ctx)
{
    return ctx->rootCa;
}

void sal_verify_server_certificates(
    Sal *ctx, bool_t verify)
{
...
static int extract_received_rport(
    osip_message_t *msg, const char **received, int *rportval, SalTransport *transport)
{
    osip_via_t           *via   = NULL;
...
static void set_sdp(
    osip_message_t *sip, sdp_message_t *msg)
{
    int  sdplen;
    char clen[10];
    char *sdp = NULL;
...
static void set_sdp_from_desc(
    osip_message_t *sip, const SalMediaDescription *desc)
{
...
int sal_call_is_offerer(
    const SalOp *h)
{
    return h->sdp_offering;
}

int sal_call_set_local_media_description(
    SalOp *h, SalMediaDescription *desc)
{
    if (desc)
...
int sal_call(
    SalOp *h, const char *from, const char *to)
{
    int            err;
    const char     *route;
...
int sal_call_decline(
    SalOp *h, SalReason reason, const char *redirect)
{
...
int sal_call_set_referer(
    SalOp *h, SalOp *refered_call)
{
...
static int send_notify_for_refer(
    int did, const char *sipfrag)
{
    osip_message_t *msg;
    eXosip_lock();
...
int sal_call_notify_refer_state(
    SalOp *h, SalOp *newcall)
{
    if (newcall == NULL)
    {
        /* in progress*/
...
int sal_ping(
    SalOp *op, const char *from, const char *to)
{
...
int sal_call_refer_with_replaces(
    SalOp *h, SalOp *other_call_h)
{
    osip_message_t *msg         = NULL;
    char           referto[256] = {0};
    int            err          = 0;
...
static void push_auth_to_exosip(
    const SalAuthInfo *info)
{
    const char *userid;
...
void sal_op_authenticate(
    SalOp *h, const SalAuthInfo *info)
{
...
void sal_op_cancel_authentication(
    SalOp *h)
{
    if (h->rid > 0)
    {
...
static void set_network_origin(
    SalOp *op, osip_message_t *req)
{
...
static void set_remote_contact(
    SalOp *op, osip_message_t *req)
{
...
static SalOp *find_op(
    Sal *sal, eXosip_event_t *ev)
{
    if (ev->cid > 0)
    {
    ...
    if (ev->response) return sal_find_other(sal, ev->response);
        ...
        return sal_find_other(sal, ev->request);
    return NULL;
}

static void inc_new_call(
    Sal *sal, eXosip_event_t *ev)
{
...
static void update_contact_from_response(
    SalOp *op, osip_message_t *response)
{
    const char   *received;
    int          rport;
    SalTransport transport;
...
static int get_auth_data_from_response(
    osip_message_t *resp, const char **realm, const char **username)
{
...
static int get_auth_data_from_request(
    osip_message_t *msg, const char **realm, const char **username)
{
...
static int get_auth_data(
    eXosip_event_t *ev, const char **realm, const char **username)
{
...
int sal_op_get_auth_requested(
    SalOp *op, const char **realm, const char **username)
{
    if (op->pending_auth)
    {
...
static bool_t process_authentication(
    Sal *sal, eXosip_event_t *ev)
{
    SalOp      *op;
...
static void process_media_control_xml(
    Sal *sal, eXosip_event_t *ev)
{
    SalOp       *op   = find_op(sal, ev);
...
static void process_dtmf_relay(
    Sal *sal, eXosip_event_t *ev)
{
    SalOp       *op   = find_op(sal, ev);
    osip_body_t *body = NULL;
...
static void fill_options_answer(
...
static void process_refer(
    Sal *sal, SalOp *op, eXosip_event_t *ev)
{
    osip_header_t  *h   = NULL;
    osip_message_t *ans = NULL;
...
static bool_t comes_from_local_if(
    osip_message_t *msg)
{
    osip_via_t *via = NULL;
    osip_message_get_via(msg, 0, &via);
...
static void masquerade_via(
    osip_message_t *msg, const char *ip, const char *port)
{
    osip_via_t *via = NULL;
    osip_message_get_via(msg, 0, &via);
    if (via)
...
static bool_t fix_message_contact(
    SalOp *op, osip_message_t *request, osip_message_t *last_answer, bool_t expire_last_contact)
{
...
static bool_t register_again_with_updated_contact(
    SalOp *op, osip_message_t *orig_request, osip_message_t *last_answer)
{
    osip_contact_t *ctt                 = NULL;
...
static void registration_success(
    Sal *sal, eXosip_event_t *ev)
{
...
static bool_t registration_failure(
    Sal *sal, eXosip_event_t *ev)
{
    int        status_code = 0;
    const char *reason     = NULL;
...
static void other_request_reply(
    Sal *sal, eXosip_event_t *ev)
{
    SalOp *op = find_op(sal, ev);
    if (op == NULL)
...
static void process_in_call_reply(
    Sal *sal, eXosip_event_t *ev)
{
    SalOp *op = find_op(sal, ev);
    if (ev->response)
...
static void register_set_contact(
    osip_message_t *msg, const char *contact)
{
...
void sal_message_add_route(
    osip_message_t *msg, const char *proxy)
{
    osip_route_t *route;
...
int sal_register(
    SalOp *h, const char *proxy, const char *from, int expires)
{
    osip_message_t *msg;
    const char     *contact = sal_op_get_contact(h);
...
const char *sal_address_get_scheme(
    const SalAddress *addr)
{
...
const char *sal_address_get_username(
    const SalAddress *addr)
{
    const osip_from_t *u = (const osip_from_t *)addr;
...
const char *sal_address_get_domain(
    const SalAddress *addr)
{
    const osip_from_t *u = (const osip_from_t *)addr;
...
void sal_address_set_display_name(
    SalAddress *addr, const char *display_name)
{
...
void sal_address_set_username(
    SalAddress *addr, const char *username)
{
...
void sal_address_set_domain(
    SalAddress *addr, const char *host)
{
    osip_from_t *uri = (osip_from_t *)addr;
...
void sal_address_set_port(
    SalAddress *addr, const char *port)
{
    osip_from_t *uri = (osip_from_t *)addr;
...
void sal_address_set_port_int(
    SalAddress *uri, int port)
{
    char tmp[12];
    if (port == 5060)
    {
...
char *sal_address_as_string_uri_only(
    const SalAddress *u)
{
    char *tmp = NULL, *ret;
...
void sal_address_set_param(
    SalAddress *u, const char *name, const char *value)
{
    osip_uri_param_t *param = NULL;
...
const char *sal_address_get_port(
    const SalAddress *addr)
{
    const osip_from_t *u = (const osip_from_t *)addr;
...
int sal_address_get_port_int(
    const SalAddress *uri)
{
...
SalTransport sal_address_get_transport(
    const SalAddress *addr)
{
    const osip_from_t *u               = (const osip_from_t *)addr;
...
void sal_address_set_transport(
    SalAddress *addr, SalTransport transport)
{
...
void sal_reuse_authorization(
    Sal *ctx, bool_t value)
{
    ctx->reuse_authorization = value;
}

void sal_exosip_add_custom_headers(
    osip_message_t *msg, SalCustomHeader *ch)
{
...
SalCustomHeader *sal_exosip_get_custom_headers(
    osip_message_t *msg)
{
    int             i    = 0;
