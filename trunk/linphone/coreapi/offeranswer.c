...
static PayloadType *find_payload_type_best_match(
    const MSList *l, const PayloadType *refpt)
{
    PayloadType  *pt;
    char         value[10];
    const MSList *elem;
...
static MSList *match_payloads(
    const MSList *local, const MSList *remote, bool_t reading_response, bool_t one_matching_codec)
{
    const MSList *e2, *e1;
    MSList       *res = NULL;
    PayloadType  *matched;
...
static bool_t match_crypto_algo(
    const SalSrtpCryptoAlgo *local, const SalSrtpCryptoAlgo *remote,
...
static SalStreamDir compute_dir_outgoing(
    SalStreamDir local, SalStreamDir answered)
{
...
static SalStreamDir compute_dir_incoming(
    SalStreamDir local, SalStreamDir offered)
{
 ...
 **/
int offer_answer_initiate_outgoing(
    const SalMediaDescription *local_offer,
 ...
 **/
int offer_answer_initiate_incoming(
    const SalMediaDescription *local_capabilities,
