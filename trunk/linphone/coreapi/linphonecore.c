...
void lc_callback_obj_init(
    LCCallbackObj *obj, LinphoneCoreCbFunc func, void *ud)
{
    obj->_func      = func;
    obj->_user_data = ud;
}

int lc_callback_obj_invoke(
    LCCallbackObj *obj, LinphoneCore *lc)
{
...
void call_logs_write_to_config_file(
    LinphoneCore *lc)
{
    MSList   *elem;
    char     logsection[32];
...
static void call_logs_read_from_config_file(
    LinphoneCore *lc)
{
    char       logsection[32];
    int        i;
...
const char *linphone_call_log_get_call_id(
    const LinphoneCallLog *cl)
{
    return cl->call_id;
}
 ...
 **/
void linphone_call_log_set_user_pointer(
    LinphoneCallLog *cl, void *up)
{
    cl->user_pointer = up;
}
 ...
 **/
void *linphone_call_log_get_user_pointer(
    const LinphoneCallLog *cl)
{
 ...
 *
 **/
void linphone_call_log_set_ref_key(
    LinphoneCallLog *cl, const char *refkey)
{
    if (cl->refkey != NULL)
    {
        ms_free(cl->refkey);
 ...
 **/
LinphoneAddress *linphone_call_log_get_from(
    LinphoneCallLog *cl)
{
    return cl->from;
}
 ...
 **/
LinphoneAddress *linphone_call_log_get_to(
    LinphoneCallLog *cl)
{
    return cl->to;
}
 ...
 **/
LinphoneAddress *linphone_call_log_get_remote_address(
    LinphoneCallLog *cl)
{
 ...
 **/
LinphoneCallDir linphone_call_log_get_dir(
    LinphoneCallLog *cl)
{
    return cl->dir;
}
 ...
 **/
time_t linphone_call_log_get_start_date(
    LinphoneCallLog *cl)
{
 ...
 **/
int linphone_call_log_get_duration(
    LinphoneCallLog *cl)
{
    return cl->duration;
}
 ...
 *
 **/
bool_t linphone_call_asked_to_autoanswer(
    LinphoneCall *call)
{
...
int linphone_core_get_current_call_duration(
...
void linphone_core_set_log_file(
    FILE *file)
{
...
void linphone_core_set_log_level(
    OrtpLogLevel loglevel)
{
 ...
 *
 **/
void linphone_core_enable_logs(
    FILE *file)
{
    if (file == NULL) file = stdout;
    ortp_set_log_file(file);
 ...
 *
 **/
// 啟用 log，採用用戶傳入的 function 輸出 log
void linphone_core_enable_logs_with_cb(
    OrtpLogFunc logfunc)
{
    ortp_set_log_level_mask(ORTP_MESSAGE | ORTP_WARNING | ORTP_ERROR | ORTP_FATAL);
    ortp_set_log_handler(logfunc);
...
static void build_sound_devices_table(
    LinphoneCore *lc)
{
    const char   **devices;
    const char   **old;
...
static PayloadType *find_payload(
    RtpProfile *prof, const char *mime_type, int clock_rate, int channels, const char *recv_fmtp)
{
...
static bool_t get_codec(
    LinphoneCore *lc, const char *type, int index, PayloadType **ret)
{
    char        codeckey[50];
...
static int codec_compare(
    const PayloadType *a, const PayloadType *b)
{
    int ra, rb;
...
static MSList *add_missing_codecs(
    LinphoneCore *lc, SalStreamType mtype, MSList *l)
{
    int i;
    for (i = 0; i < RTP_PROFILE_MAX_PAYLOADS; ++i)
    {
...
static MSList *codec_append_if_new(
    MSList *l, PayloadType *pt)
{
    MSList *elem;
...
static void codecs_config_read(
    LinphoneCore *lc)
{
    int         i;
    PayloadType *pt;
...
static void build_video_devices_table(
    LinphoneCore *lc)
{
    const MSList *elem;
    int          i;
 ...
 **/
void linphone_core_enable_adaptive_rate_control(
 ...
 **/
bool_t linphone_core_adaptive_rate_control_enabled(
    const LinphoneCore *lc)
{
    return lp_config_get_int(lc->config, "net", "adaptive_rate_control", TRUE);
}

bool_t linphone_core_rtcp_enabled(
    const LinphoneCore *lc)
{
 ...
 */
void linphone_core_set_download_bandwidth(
    LinphoneCore *lc, int bw)
{
 ...
 * @param bw the bandwidth in kbits/s, 0 for infinite
 */
void linphone_core_set_upload_bandwidth(
    LinphoneCore *lc, int bw)
{
 ...
 * @ingroup media_parameters
 */
void linphone_core_set_download_ptime(
    LinphoneCore *lc, int ptime)
{
 ...
 * @ingroup media_parameters
 **/
void linphone_core_set_upload_ptime(
    LinphoneCore *lc, int ptime)
{
 ...
 * @ingroup media_parameters
 **/
int linphone_core_get_upload_ptime(
    LinphoneCore *lc)
{
...
static void linphone_core_free_payload_types(
    LinphoneCore *lc)
{
...
static void linphone_core_init(
    LinphoneCore *lc, const LinphoneCoreVTable *vtable, LpConfig *config, void *userdata)
{
    ...
    linphone_core_assign_payload_type(lc, &payload_type_g726_32,         19,  NULL);
    linphone_core_assign_payload_type(lc, &payload_type_bv16,            106, NULL);
    linphone_core_assign_payload_type(lc, &payload_type_bv32,            107, NULL);
    linphone_core_assign_payload_type(lc, &payload_type_lpc1015,         115, NULL);
    ...
    linphone_core_assign_payload_type(lc, &payload_type_ilbc,            113, "mode=30");
    linphone_core_assign_payload_type(lc, &payload_type_amr,             114, "octet-align=1");
    ...
    linphone_core_assign_payload_type(lc, &payload_type_x_snow,    100, NULL);
    linphone_core_assign_payload_type(lc, &payload_type_h264,      102, NULL);
    linphone_core_assign_payload_type(lc, &payload_type_h264,      103, "packetization-mode=1");
#endif
 ...
 *               一個LinphoneCoreVTable結構持有你的應用程序回調
 ...
 *        一個配置文件的路徑。如果它不存在，它將被創建。配置文件用於存儲所有用戶設置，通話記錄，朋友，代理...
 ...
 *
 **/
LinphoneCore *linphone_core_new(
...
LinphoneCore *linphone_core_new_with_config(
...
int linphone_core_set_primary_contact(
    LinphoneCore *lc, const char *contact)
{
    LinphoneAddress *ctt;
...
void linphone_core_get_local_ip(
    LinphoneCore *lc, const char *dest, char *result)
{
    const char *ip;
...
static void update_primary_contact(
    LinphoneCore *lc)
{
    char            *guessed = NULL;
 ...
 * This list is made of struct PayloadType describing the codec parameters.
 **/
int linphone_core_set_video_codecs(
    LinphoneCore *lc, MSList *codecs)
{
...
void linphone_core_enable_audio_adaptive_jittcomp(
    LinphoneCore *lc, bool_t val)
{
    lc->rtp_conf.audio_adaptive_jitt_comp_enabled = val;
}

bool_t linphone_core_audio_adaptive_jittcomp_enabled(
    LinphoneCore *lc)
{
...
void linphone_core_enable_video_adaptive_jittcomp(
    LinphoneCore *lc, bool_t val)
{
    lc->rtp_conf.video_adaptive_jitt_comp_enabled = val;
}

bool_t linphone_core_video_adaptive_jittcomp_enabled(
    LinphoneCore *lc)
{
 ...
 * @ingroup media_parameters
 **/
int linphone_core_get_video_jittcomp(
    LinphoneCore *lc)
{
 ...
 * @ingroup network_parameters
 */
void linphone_core_get_audio_port_range(
    const LinphoneCore *lc, int *min_port, int *max_port)
{
 ...
 * @ingroup network_parameters
 */
void linphone_core_get_video_port_range(
    const LinphoneCore *lc, int *min_port, int *max_port)
{
...
bool_t linphone_core_get_rtp_no_xmit_on_audio_mute(
    const LinphoneCore *lc)
{
 ...
 * @ingroup media_parameters
 **/
void linphone_core_set_video_jittcomp(
    LinphoneCore *lc, int value)
{
    lc->rtp_conf.video_jitt_comp = value;
}

void linphone_core_set_rtp_no_xmit_on_audio_mute(
    LinphoneCore *lc, bool_t rtp_no_xmit_on_audio_mute)
{
 ...
 * @ingroup network_parameters
 **/
void linphone_core_set_audio_port(
    LinphoneCore *lc, int port)
{
 ...
 * @ingroup media_parameters
 */
void linphone_core_set_audio_port_range(
    LinphoneCore *lc, int min_port, int max_port)
{
 ...
 * @ingroup media_parameters
 */
void linphone_core_set_video_port_range(
    LinphoneCore *lc, int min_port, int max_port)
{
 ...
 **/
void linphone_core_set_nortp_timeout(
    LinphoneCore *lc, int nortp_timeout)
{
 ...
 * @ingroup media_parameters
 **/
void linphone_core_set_use_info_for_dtmf(
    LinphoneCore *lc, bool_t use_info)
{
 ...
 * @ingroup media_parameters
 **/
bool_t linphone_core_get_use_rfc2833_for_dtmf(
    LinphoneCore *lc)
{
 ...
 * @ingroup media_parameters
 **/
void linphone_core_set_use_rfc2833_for_dtmf(
    LinphoneCore *lc, bool_t use_rfc2833)
{
    if (linphone_core_ready(lc))
    {
 ...
 * @ingroup misc
 **/
void linphone_core_set_user_agent(
    LinphoneCore *lc, const char *name, const char *ver)
{
...
static void transport_error(
    LinphoneCore *lc, const char *transport, int port)
{
...
static bool_t transports_unchanged(
    const LCSipTransports *tr1, const LCSipTransports *tr2)
{
    return
 ...
 * @ingroup network_parameters
 **/
int linphone_core_set_sip_transports(
    LinphoneCore *lc, const LCSipTransports *tr)
{
 ...
 * @ingroup network_parameters
 **/
int linphone_core_get_sip_transports(
    LinphoneCore *lc, LCSipTransports *tr)
{
 ...
 * @ingroup network_parameters
 **/
void linphone_core_set_sip_port(
    LinphoneCore *lc, int port)
{
    LCSipTransports tr;
    memset(&tr, 0, sizeof(tr));
 ...
 **/
void linphone_core_enable_ipv6(
    LinphoneCore *lc, bool_t val)
{
...
static void monitor_network_state(
    LinphoneCore *lc, time_t curtime)
{
...
static void assign_buddy_info(
    LinphoneCore *lc, BuddyInfo *info)
{
...
static void analyze_buddy_lookup_results(
...
static void linphone_core_grab_buddy_infos(
    LinphoneCore *lc, LinphoneProxyConfig *cfg)
{
    const MSList    *elem;
    SipSetupContext *ctx = linphone_proxy_config_get_sip_setup_context(cfg);
...
static void linphone_core_do_plugin_tasks(
    LinphoneCore *lc)
{
...
LinphoneAddress *linphone_core_interpret_url(
...
void linphone_core_start_refered_call(
...
void linphone_core_notify_refer_state(
    LinphoneCore *lc, LinphoneCall *referer, LinphoneCall *newcall)
{
    if (referer->op != NULL)
    {
...
LinphoneProxyConfig *linphone_core_lookup_known_proxy(
    LinphoneCore *lc, const LinphoneAddress *uri)
{
    const MSList        *elem;
...
const char *linphone_core_find_best_identity(
    LinphoneCore *lc, const LinphoneAddress *to, const char **route)
{
...
static char *get_fixed_contact(
    LinphoneCore *lc, LinphoneCall *call, LinphoneProxyConfig *dest_proxy)
{
    LinphoneAddress *ctt;
...
int linphone_core_proceed_with_invite_if_ready(
    LinphoneCore *lc, LinphoneCall *call, LinphoneProxyConfig *dest_proxy)
{
...
int linphone_core_start_invite(
    LinphoneCore *lc, LinphoneCall *call)
{
    int                 err;
    char                *contact;
 ...
 **/
LinphoneCall *linphone_core_invite(
    LinphoneCore *lc, const char *url)
{
    LinphoneCall       *call;
 ...
 * @return a LinphoneCall object or NULL in case of failure
 **/
LinphoneCall *linphone_core_invite_with_params(
    LinphoneCore *lc, const char *url, const LinphoneCallParams *p)
{
...
LinphoneCall *linphone_core_invite_address(
    LinphoneCore *lc, const LinphoneAddress *addr)
{
    LinphoneCall       *call;
    LinphoneCallParams *p = linphone_core_create_default_call_parameters(lc);
    p->has_video &= !!lc->video_policy.automatically_initiate;
 ...
 **/
int linphone_core_transfer_call(
    LinphoneCore *lc, LinphoneCall *call, const char *url)
{
    char            *real_url        = NULL;
 ...
 **/
int linphone_core_transfer_call_to_another(
    LinphoneCore *lc, LinphoneCall *call, LinphoneCall *dest)
{
...
bool_t linphone_core_incompatible_security(
    LinphoneCore *lc, SalMediaDescription *md)
{
...
void linphone_core_notify_incoming_call(
    LinphoneCore *lc, LinphoneCall *call)
{
    char                *barmesg;
    char                *tmp;
    ...
    if (lc->vtable.inv_recv)
        lc->vtable.inv_recv(lc, tmp);
    ms_free(barmesg);
    ms_free(tmp);
}

int linphone_core_start_update_call(
    LinphoneCore *lc, LinphoneCall *call)
{
    const char *subject;
 ...
 **/
int linphone_core_defer_call_update(
    LinphoneCore *lc, LinphoneCall *call)
{
...
int linphone_core_start_accept_call_update(
    LinphoneCore *lc, LinphoneCall *call)
{
    SalMediaDescription *md;
    if (call->ice_session != NULL)
 ...
 **/
int linphone_core_accept_call_update(
    LinphoneCore *lc, LinphoneCall *call, const LinphoneCallParams *params)
{
...
int linphone_core_abort_call(
    LinphoneCore *lc, LinphoneCall *call, const char *error)
{
...
int linphone_core_redirect_call(
    LinphoneCore *lc, LinphoneCall *call, const char *redirect_uri)
{
    if (call->state == LinphoneCallIncomingReceived)
    {
 ...
 **/
int linphone_core_terminate_call(
    LinphoneCore *lc, LinphoneCall *the_call)
{
    LinphoneCall *call;
    if (the_call == NULL)
    {
 ...
 **/
int linphone_core_decline_call(
    LinphoneCore *lc, LinphoneCall *call, LinphoneReason reason)
{
 ...
 * @param lc The LinphoneCore
 **/
int linphone_core_terminate_all_calls(
    LinphoneCore *lc)
{
 ...
 * @ingroup call_control
 **/
const MSList *linphone_core_get_calls(
    LinphoneCore *lc)
{
    return lc->calls;
}
 ...
 * @ingroup call_control
 **/
LinphoneCall *linphone_core_get_current_call(
    const LinphoneCore *lc)
{
 ...
 * @ingroup call_control
 **/
int linphone_core_pause_all_calls(
    LinphoneCore *lc)
{
    const MSList *elem;
...
void linphone_core_preempt_sound_resources(
    LinphoneCore *lc)
{
...
static int remote_address_compare(
    LinphoneCall *call, const LinphoneAddress *raddr)
{
...
LinphoneCall *linphone_core_get_call_by_remote_address(
    LinphoneCore *lc, const char *remote_address)
{
...
int linphone_core_send_publish(
    LinphoneCore         *lc,
 ...
 **/
void linphone_core_set_inc_timeout(
    LinphoneCore *lc, int seconds)
{
 ...
 **/
void linphone_core_set_in_call_timeout(
    LinphoneCore *lc, int seconds)
{
 ...
 **/
int linphone_core_get_in_call_timeout(
    LinphoneCore *lc)
{
 ...
 **/
int linphone_core_get_delayed_timeout(
    LinphoneCore *lc)
{
 ...
 **/
void linphone_core_set_delayed_timeout(
    LinphoneCore *lc, int seconds)
{
    lc->sip_conf.delayed_timeout = seconds;
}

void linphone_core_set_presence_info(
    LinphoneCore *lc, int minutes_away,
    const char *contact,
    LinphoneOnlineStatus presence_mode)
{
 ...
 * @ingroup media_parameters
 **/
void linphone_core_set_ring_level(
    LinphoneCore *lc, int level)
{
    MSSndCard *sndcard;
 ...
 * @ingroup media_parameters
 **/
float linphone_core_get_mic_gain_db(
    LinphoneCore *lc)
{
 ...
 * @ingroup media_parameters
 **/
float linphone_core_get_playback_gain_db(
    LinphoneCore *lc)
{
 ...
 * @ingroup media_parameters
 **/
void linphone_core_set_play_level(
    LinphoneCore *lc, int level)
{
    MSSndCard *sndcard;
 ...
 * @ingroup media_parameters
 **/
void linphone_core_set_rec_level(
    LinphoneCore *lc, int level)
{
    MSSndCard *sndcard;
...
static MSSndCard *get_card_from_string_id(
    const char *devid, unsigned int cap)
{
...
bool_t linphone_core_sound_device_can_capture(
    LinphoneCore *lc, const char *devid)
{
    MSSndCard *sndcard;
    sndcard = ms_snd_card_manager_get_card(ms_snd_card_manager_get(), devid);
 ...
 * @param devid the device name as returned by linphone_core_get_sound_devices()
 **/
bool_t linphone_core_sound_device_can_playback(
    LinphoneCore *lc, const char *devid)
{
    MSSndCard *sndcard;
    sndcard = ms_snd_card_manager_get_card(ms_snd_card_manager_get(), devid);
 ...
 * @param devid the device name as returned by linphone_core_get_sound_devices()
 **/
int linphone_core_set_ringer_device(
    LinphoneCore *lc, const char *devid)
{
 ...
 * @param devid the device name as returned by linphone_core_get_sound_devices()
 **/
int linphone_core_set_playback_device(
    LinphoneCore *lc, const char *devid)
{
    MSSndCard *card = get_card_from_string_id(devid, MS_SND_CARD_CAP_PLAYBACK);
 ...
 * @param devid the device name as returned by linphone_core_get_sound_devices()
 **/
int linphone_core_set_capture_device(
    LinphoneCore *lc, const char *devid)
{
 ...
 **/
void linphone_core_reload_sound_devices(
    LinphoneCore *lc)
{
...
void linphone_core_set_sound_source(
    LinphoneCore *lc, char source)
{
 ...
 * @ingroup media_parameters
 **/
void linphone_core_set_ring(
    LinphoneCore *lc, const char *path)
{
 ...
 * @ingroup initializing
 **/
void linphone_core_set_root_ca(
    LinphoneCore *lc, const char *path)
{
 ...
 * @ingroup initializing
 **/
const char *linphone_core_get_root_ca(
    LinphoneCore *lc)
{
 ...
 * @ingroup initializing
 **/
void linphone_core_verify_server_certificates(
    LinphoneCore *lc, bool_t yesno)
{
...
static void notify_end_of_ring(
    void *ud, MSFilter *f, unsigned int event, void *arg)
{
    LinphoneCore *lc = (LinphoneCore *)ud;
    lc->preview_finished = 1;
}

int linphone_core_preview_ring(
    LinphoneCore *lc, const char *ring, LinphoneCoreCbFunc func, void *userdata)
{
    if (lc->ringstream != 0)
    {
 ...
 * @ingroup media_parameters
 **/
void linphone_core_enable_echo_cancellation(
    LinphoneCore *lc, bool_t val)
{
 ...
 * @ingroup media_parameters
 **/
bool_t linphone_core_echo_cancellation_enabled(
    LinphoneCore *lc)
{
    return lc->sound_conf.ec;
}

void linphone_core_enable_echo_limiter(
    LinphoneCore *lc, bool_t val)
{
 ...
 **/
bool_t linphone_core_is_mic_muted(
    LinphoneCore *lc)
{
    LinphoneCall *call = linphone_core_get_current_call(lc);
    if (linphone_core_is_in_conference(lc))
...
bool_t linphone_core_is_rtp_muted(
    LinphoneCore *lc)
{
    LinphoneCall *call = linphone_core_get_current_call(lc);
    if (call == NULL)
 ...
 *
 **/
void linphone_core_send_dtmf(
    LinphoneCore *lc, char dtmf)
{
    LinphoneCall *call = linphone_core_get_current_call(lc);
    if (call == NULL)
    {
...
void linphone_core_set_stun_server(
    LinphoneCore *lc, const char *server)
{
...
LinphoneUpnpState linphone_core_get_upnp_state(
    const LinphoneCore *lc)
{
#ifdef BUILD_UPNP
...
const char *linphone_core_get_upnp_external_ipaddress(
    const LinphoneCore *lc)
{
#ifdef BUILD_UPNP
...
int linphone_core_set_relay_addr(
    LinphoneCore *lc, const char *addr)
{
...
void linphone_core_set_nat_address(
    LinphoneCore *lc, const char *addr)
{
...
const char *linphone_core_get_nat_address_resolved(
    LinphoneCore *lc)
{
...
void linphone_core_set_firewall_policy(
    LinphoneCore *lc, LinphoneFirewallPolicy pol)
{
#ifndef BUILD_UPNP
 ...
 * @ingroup call_logs
 **/
const MSList *linphone_core_get_call_logs(
    LinphoneCore *lc)
{
    return lc->call_logs;
}
 ...
 * @ingroup call_logs
 **/
void linphone_core_clear_call_logs(
    LinphoneCore *lc)
{
    lc->missed_calls = 0;
 ...
 **/
int linphone_core_get_missed_calls_count(
    LinphoneCore *lc)
{
 ...
 **/
void linphone_core_reset_missed_calls_count(
    LinphoneCore *lc)
{
    lc->missed_calls = 0;
}
 ...
 **/
void linphone_core_remove_call_log(
    LinphoneCore *lc, LinphoneCallLog *cl)
{
...
static void toggle_video_preview(
    LinphoneCore *lc, bool_t val)
{
#ifdef VIDEO_ENABLED
    if (val)
 ...
 *
 **/
void linphone_core_enable_video(
    LinphoneCore *lc, bool_t vcap_enabled, bool_t display_enabled)
{
...
bool_t linphone_core_video_supported(
    LinphoneCore *lc)
{
#ifdef VIDEO_ENABLED
    return TRUE;
 ...
 * @ingroup media_parameters
 **/
void linphone_core_set_video_policy(
    LinphoneCore *lc, const LinphoneVideoPolicy *policy)
{
 ...
 * @ingroup media_parameters
 **/
const LinphoneVideoPolicy *linphone_core_get_video_policy(
    LinphoneCore *lc)
{
 ...
 **/
void linphone_core_enable_video_preview(
    LinphoneCore *lc, bool_t val)
{
 ...
 **/
void linphone_core_enable_self_view(
    LinphoneCore *lc, bool_t val)
{
#ifdef VIDEO_ENABLED
    LinphoneCall *call = linphone_core_get_current_call(lc);
 ...
 **/
int linphone_core_set_video_device(
...
#ifdef VIDEO_ENABLED
static VideoStream *get_active_video_stream(
    LinphoneCore *lc)
{
    VideoStream  *vs   = NULL;
    LinphoneCall *call = linphone_core_get_current_call(lc);
...
int linphone_core_set_static_picture(
    LinphoneCore *lc, const char *path)
{
#ifdef VIDEO_ENABLED
...
const char *linphone_core_get_static_picture(
    LinphoneCore *lc)
{
...
float linphone_core_get_static_picture_fps(
    LinphoneCore *lc)
{
#ifdef VIDEO_ENABLED
    VideoStream *vs = NULL;
...
void linphone_core_set_native_video_window_id(
    LinphoneCore *lc, unsigned long id)
{
#ifdef VIDEO_ENABLED
    LinphoneCall *call = linphone_core_get_current_call(lc);
 ...
 * @ingroup media_parameters
 **/
unsigned long linphone_core_get_native_preview_window_id(
    const LinphoneCore *lc)
{
#ifdef VIDEO_ENABLED
    LinphoneCall *call = linphone_core_get_current_call(lc);
 ...
 * If not set the core will create its own window.
 **/
void linphone_core_set_native_preview_window_id(
    LinphoneCore *lc, unsigned long id)
{
#ifdef VIDEO_ENABLED
    LinphoneCall *call = linphone_core_get_current_call(lc);
 ...
 **/
void linphone_core_show_video(
    LinphoneCore *lc, bool_t show)
{
#ifdef VIDEO_ENABLED
    LinphoneCall *call = linphone_core_get_current_call(lc);
 ...
 *
 **/
void linphone_core_use_preview_window(
    LinphoneCore *lc, bool_t yesno)
{
 ...
 *
 **/
void linphone_core_set_device_rotation(
    LinphoneCore *lc, int rotation)
{
...
int linphone_core_get_camera_sensor_rotation(
    LinphoneCore *lc)
{
#ifdef VIDEO_ENABLED
    LinphoneCall *call = linphone_core_get_current_call(lc);
...
static MSVideoSize video_size_get_by_name(
    const char *name)
{
...
static const char *video_size_get_name(
    MSVideoSize vsize)
{
    MSVideoSizeDef *pdef = supported_resolutions;
    for (; pdef->name != NULL; pdef++)
...
static bool_t video_size_supported(
    MSVideoSize vsize)
{
 ...
 **/
void linphone_core_set_preferred_video_size(
    LinphoneCore *lc, MSVideoSize vsize)
{
 ...
 **/
void linphone_core_set_preferred_video_size_by_name(
    LinphoneCore *lc, const char *name)
{
 ...
 **/
void linphone_core_set_play_file(
 ...
 **/
void linphone_core_play_dtmf(
    LinphoneCore *lc, char dtmf, int duration_ms)
{
...
void linphone_core_play_named_tone(
    LinphoneCore *lc, LinphoneToneID toneid)
{
 ...
 * @ingroup initializing
 **/
void linphone_core_set_user_data(
    LinphoneCore *lc, void *userdata)
{
    lc->data = userdata;
}
 ...
 * @ingroup media_parameters
 **/
void linphone_core_set_mtu(
    LinphoneCore *lc, int mtu)
{
    lc->net_conf.mtu = mtu;
    if (mtu > 0)
    {
...
void linphone_core_set_waiting_callback(
    LinphoneCore *lc, LinphoneWaitingCallback cb, void *user_context)
{
    lc->wait_cb  = cb;
    lc->wait_ctx = user_context;
}

void linphone_core_start_waiting(
    LinphoneCore *lc, const char *purpose)
{
    if (lc->wait_cb)
    {
...
void linphone_core_update_progress(
    LinphoneCore *lc, const char *purpose, float progress)
{
    if (lc->wait_cb)
    {
...
void linphone_core_set_rtp_transport_factories(
    LinphoneCore *lc, LinphoneRtpTransportFactories *factories)
{
    lc->rtptf = factories;
}
...
int linphone_core_get_current_call_stats(
    LinphoneCore *lc, rtp_stats_t *local, rtp_stats_t *remote)
{
    LinphoneCall *call = linphone_core_get_current_call(lc);
    if (call != NULL)
    {
...
static void sound_config_uninit(
    LinphoneCore *lc)
{
...
static void video_config_uninit(
    LinphoneCore *lc)
{
...
void _linphone_core_codec_config_write(
    LinphoneCore *lc)
{
    if (linphone_core_ready(lc))
    {
...
static void codecs_config_uninit(
    LinphoneCore *lc)
{
    _linphone_core_codec_config_write(lc);
 ...
 *
 **/
LpConfig *linphone_core_get_config(
    LinphoneCore *lc)
{
    return lc->config;
}

static void linphone_core_uninit(
    LinphoneCore *lc)
{
...
static void set_network_reachable(
    LinphoneCore *lc, bool_t isReachable, time_t curtime)
{
...
bool_t linphone_core_is_network_reachable(
    LinphoneCore *lc)
{
    return lc->network_reachable;
}

ortp_socket_t linphone_core_get_sip_socket(
    LinphoneCore *lc)
{
 ...
 **/
void linphone_core_set_remote_ringback_tone(
    LinphoneCore *lc, const char *file)
{
 ...
 **/
const char *linphone_core_get_remote_ringback_tone(
    const LinphoneCore *lc)
{
    return lc->sound_conf.ringback_tone;
}

static PayloadType *find_payload_type_from_list(
    const char *type, int rate, int channels, const MSList *from)
{
    const MSList *elem;
...
PayloadType *linphone_core_find_payload_type(
    LinphoneCore *lc, const char *type, int rate, int channels)
{
...
const char *linphone_global_state_to_string(
    LinphoneGlobalState gs)
{
    switch (gs)
    {
...
LinphoneGlobalState linphone_core_get_global_state(
    const LinphoneCore *lc)
{
    return lc->state;
}

LinphoneCallParams *linphone_core_create_default_call_parameters(
    LinphoneCore *lc)
{
...
const char *linphone_reason_to_string(
    LinphoneReason err)
{
    switch (err)
    {
...
const char *linphone_error_to_string(
    LinphoneReason err)
{
 ...
 */
void linphone_core_enable_keep_alive(
    LinphoneCore *lc, bool_t enable)
{
#ifdef BUILD_UPNP
 ...
 */
bool_t linphone_core_keep_alive_enabled(
    LinphoneCore *lc)
{
    return sal_get_keepalive_period(lc->sal) > 0;
}

void linphone_core_start_dtmf_stream(
    LinphoneCore *lc)
{
...
void linphone_core_stop_dtmf_stream(
    LinphoneCore *lc)
{
    if (lc->ringstream && lc->dmfs_playing_start_time != 0)
    {
...
int linphone_core_get_max_calls(
    LinphoneCore *lc)
{
    return lc->max_calls;
}

void linphone_core_set_max_calls(
    LinphoneCore *lc, int max)
{
    lc->max_calls = max;
}
...
static Hook *hook_new(
    LinphoneCoreIterateHook hook, void *hook_data)
{
...
static void hook_invoke(
    Hook *h)
{
    h->fun(h->data);
}

void linphone_core_add_iterate_hook(
    LinphoneCore *lc, LinphoneCoreIterateHook hook, void *hook_data)
{
    lc->hooks = ms_list_append(lc->hooks, hook_new(hook, hook_data));
}

static void linphone_core_run_hooks(
    LinphoneCore *lc)
{
    ms_list_for_each(lc->hooks, (void (*)(void *))hook_invoke);
}

static void linphone_core_free_hooks(
    LinphoneCore *lc)
{
...
void linphone_core_remove_iterate_hook(
    LinphoneCore *lc, LinphoneCoreIterateHook hook, void *hook_data)
{
    MSList *elem;
...
const char *linphone_core_get_zrtp_secrets_file(
    LinphoneCore *lc)
{
    return lc->zrtp_secrets_cache;
}

const LinphoneCall *linphone_core_find_call_from_uri(
 ...
 * @param lc The LinphoneCore
 **/
bool_t linphone_core_sound_resources_locked(
    LinphoneCore *lc)
{
    MSList *calls = lc->calls;
    while (calls)
...
void linphone_core_set_srtp_enabled(
    LinphoneCore *lc, bool_t enabled)
{
 ...
 **/
bool_t linphone_core_media_encryption_supported(
    const LinphoneCore *lc, LinphoneMediaEncryption menc)
{
    switch (menc)
    {
...
int linphone_core_set_media_encryption(
    LinphoneCore *lc, enum LinphoneMediaEncryption menc)
{
...
LinphoneMediaEncryption linphone_core_get_media_encryption(
    LinphoneCore *lc)
{
...
bool_t linphone_core_is_media_encryption_mandatory(
    LinphoneCore *lc)
{
...
void linphone_core_set_media_encryption_mandatory(
    LinphoneCore *lc, bool_t m)
{
    lp_config_set_int(lc->config, "sip", "media_encryption_mandatory", (int)m);
}

void linphone_core_init_default_params(
    LinphoneCore *lc, LinphoneCallParams *params)
{
...
void linphone_core_set_device_identifier(
    LinphoneCore *lc, const char *device_id)
{
    if (lc->device_id) ms_free(lc->device_id);
    lc->device_id = ms_strdup(device_id);
}

const char *linphone_core_get_device_identifier(
    const LinphoneCore *lc)
{
    return lc->device_id;
}
 ...
 *
 **/
void linphone_core_set_sip_dscp(
    LinphoneCore *lc, int dscp)
{
 ...
 *
 **/
int linphone_core_get_sip_dscp(
    const LinphoneCore *lc)
{
 ...
 *
 **/
void linphone_core_set_audio_dscp(
    LinphoneCore *lc, int dscp)
{
    if (linphone_core_ready(lc))
 ...
 *
 **/
int linphone_core_get_audio_dscp(
    const LinphoneCore *lc)
{
 ...
 *
 **/
void linphone_core_set_video_dscp(
    LinphoneCore *lc, int dscp)
{
    if (linphone_core_ready(lc))
