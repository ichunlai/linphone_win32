...
MSList *linphone_find_friend(
    MSList *fl, const LinphoneAddress *friend, LinphoneFriend **lf)
{
    MSList         *res = NULL;
...
LinphoneFriend *linphone_find_friend_by_inc_subscribe(
    MSList *l, SalOp *op)
{
    MSList *elem;
...
LinphoneFriend *linphone_find_friend_by_out_subscribe(
    MSList *l, SalOp *op)
{
    MSList *elem;
    for (elem = l; elem != NULL; elem = elem->next)
...
void __linphone_friend_do_subscribe(
    LinphoneFriend *fr)
{
    char                *friend        = NULL;
...
LinphoneFriend *linphone_friend_new_with_addr(
    const char *addr)
{
...
bool_t linphone_friend_in_list(
    const LinphoneFriend *lf)
{
    return lf->lc != NULL;
}

void linphone_core_interpret_friend_uri(
    LinphoneCore *lc, const char *uri, char **result)
{
...
int linphone_friend_set_addr(
    LinphoneFriend *lf, const LinphoneAddress *addr)
{
...
int linphone_friend_set_name(
    LinphoneFriend *lf, const char *name)
{
...
int linphone_friend_enable_subscribes(
    LinphoneFriend *fr, bool_t val)
{
    fr->subscribe = val;
    return 0;
}

int linphone_friend_set_inc_subscribe_policy(
    LinphoneFriend *fr, LinphoneSubscribePolicy pol)
{
    fr->pol = pol;
    return 0;
}

SalPresenceStatus linphone_online_status_to_sal(
...
void linphone_friend_notify(
    LinphoneFriend *lf, LinphoneOnlineStatus os)
{
...
static void linphone_friend_unsubscribe(
    LinphoneFriend *lf)
{
    if (lf->outsub != NULL)
    {
...
void linphone_friend_close_subscriptions(
    LinphoneFriend *lf)
{
...
const LinphoneAddress *linphone_friend_get_address(
    const LinphoneFriend *lf)
{
    return lf->uri;
}

bool_t linphone_friend_get_send_subscribe(
    const LinphoneFriend *lf)
{
    return lf->subscribe;
}
...
LinphoneOnlineStatus linphone_friend_get_status(
    const LinphoneFriend *lf)
{
    return lf->status;
}

BuddyInfo *linphone_friend_get_info(
    const LinphoneFriend *lf)
{
    return lf->info;
}

void linphone_friend_apply(
    LinphoneFriend *fr, LinphoneCore *lc)
{
    if (fr->uri == NULL)
    {
...
void linphone_friend_edit(
    LinphoneFriend *fr){}

void linphone_friend_done(
    LinphoneFriend *fr)
{
...
void linphone_core_add_friend(
    LinphoneCore *lc, LinphoneFriend *lf)
{
...
void linphone_core_remove_friend(
    LinphoneCore *lc, LinphoneFriend *fl)
{
...
void linphone_friend_set_ref_key(
    LinphoneFriend *lf, const char *key)
{
    if (lf->refkey != NULL)
    {
        ms_free(lf->refkey);
...
const char *linphone_friend_get_ref_key(
    const LinphoneFriend *lf)
{
    return lf->refkey;
}

static bool_t username_match(
...
LinphoneFriend *linphone_core_get_friend_by_address(
...
LinphoneFriend *linphone_core_get_friend_by_ref_key(
    const LinphoneCore *lc, const char *key)
{
    const MSList *elem;
...
LinphoneSubscribePolicy __policy_str_to_enum(
    const char *pol)
{
...
LinphoneProxyConfig *__index_to_proxy(
...
LinphoneFriend *linphone_friend_new_from_config_file(
    LinphoneCore *lc, int index)
{
    const char     *tmp;
    char           item[50];
...
const char *__policy_enum_to_str(
...
void linphone_friend_write_to_config_file(
    LpConfig *config, LinphoneFriend *lf, int index)
{
    char       key[50];
    char       *tmp;
    const char *refkey;
...
void linphone_core_write_friends_config(
    LinphoneCore *lc)
{
    MSList *elem;
    int    i;
