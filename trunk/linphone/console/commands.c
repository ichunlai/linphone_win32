...
static int devname_to_index(
    LinphoneCore *lc, const char *devname)
{
    const char **p;
    int        i;
...
static const char *index_to_devname(
...
static void linphonec_proxy_show(
    LinphoneCore *lc, int index)
{
    const MSList *elem;
    int          i;
...
static int
linphonec_friend_list(
    LinphoneCore *lc, char *pat)
{
    const MSList *friend;
    int          n;
...
static int
linphonec_friend_call(
    LinphoneCore *lc, unsigned int num)
{
...
linphonec_friend_add(
    LinphoneCore *lc, const char *name, const char *addr)
{
...
static void
linphonec_display_command_help(
    LPC_COMMAND *cmd)
{
...
static int lpc_cmd_acodec(
    LinphoneCore *lc, char *args)
{
    return lpc_cmd_codec(AUDIO, lc, args);
}

static int lpc_cmd_vcodec(
    LinphoneCore *lc, char *args)
{
    return lpc_cmd_codec(VIDEO, lc, args);
}

static int lpc_cmd_codec(
    int type, LinphoneCore *lc, char *args)
{
    char *arg1 = args;
    char *arg2 = NULL;
    char *ptr  = args;
...
static void linphonec_codec_list(
    int type, LinphoneCore *lc)
{
    PayloadType  *pt;
    int          index = 0;
...
static void linphonec_codec_enable(
    int type, LinphoneCore *lc, int sel_index)
{
    PayloadType  *pt;
    int          index = 0;
    const MSList *node = NULL;
...
static void linphonec_codec_disable(
    int type, LinphoneCore *lc, int sel_index)
{
    PayloadType  *pt;
    int          index = 0;
...
static int lpc_cmd_echocancellation(
    LinphoneCore *lc, char *args)
{
    char     *arg1   = args;
    char     *arg2   = NULL;
...
static int lpc_cmd_echolimiter(
    LinphoneCore *lc, char *args)
{
    if (args)
    {
...
static int lpc_cmd_playback_gain(
    LinphoneCore *lc, char *args)
{
    if (args)
    {
...
static int lpc_cmd_rtp_no_xmit_on_audio_mute(
    LinphoneCore *lc, char *args)
{
...
#ifdef VIDEO_ENABLED
static int _lpc_cmd_video_window(
    LinphoneCore *lc, char *args, bool_t is_preview)
{
    char        subcommand[64];
    int         a, b;
    int         err;
...
static int lpc_cmd_video_window(
    LinphoneCore *lc, char *args)
{
    return _lpc_cmd_video_window(lc, args, FALSE);
}

static int lpc_cmd_preview_window(
    LinphoneCore *lc, char *args)
{
...
static void lpc_display_global_state(
    LinphoneCore *lc)
{
...
static void lpc_display_call_states(
    LinphoneCore *lc)
{
    LinphoneCall *call;
    const MSList *elem;
...
static void lpc_display_proxy_states(
    LinphoneCore *lc)
{
    const MSList *elem;
...
static int zrtp_set_verified(
    LinphoneCore *lc, char *args, bool_t verified)
{
    LinphoneCall *call = linphone_core_get_current_call(lc);
...
static int lpc_cmd_zrtp_verified(
    LinphoneCore *lc, char *args)
{
    return zrtp_set_verified(lc, args, TRUE);
}

static int lpc_cmd_zrtp_unverified(
    LinphoneCore *lc, char *args)
{
