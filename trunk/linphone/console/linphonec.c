...
LinphoneCall *linphonec_get_call(
    long id)
{
 ...
 */
static void
linphonec_display_refer(
    LinphoneCore *lc, const char *refer_to)
{
...
linphonec_display_something(
    LinphoneCore *lc, const char *something)
{
    fprintf(stdout, "%s\n%s", something, prompt);
    fflush(stdout);
}
...
linphonec_display_url(
    LinphoneCore *lc, const char *something, const char *url)
{
    fprintf(stdout, "%s : %s\n", something, url);
}
...
linphonec_notify_received(
    LinphoneCore *lc, LinphoneCall *call, const char *from, const char *event)
{
...
linphonec_notify_presence_received(
    LinphoneCore *lc, LinphoneFriend *fid)
{
...
static void linphonec_call_updated(
    LinphoneCall *call)
{
...
static void linphonec_call_encryption_changed(
    LinphoneCore *lc, LinphoneCall *call, bool_t encrypted, const char *auth_token)
{
...
static void linphonec_call_state_changed(
    LinphoneCore *lc, LinphoneCall *call, LinphoneCallState st, const char *msg)
{
...
static void *prompt_reader_thread(
    void *arg)
{
    char *ret;
...
static void start_prompt_reader(
    void)
{
    ortp_thread_t th;
...
#if !defined(_WIN32_WCE)
static ortp_pipe_t create_server_socket(
    void)
{
    char path[128];
    #ifndef WIN32
...
void linphonec_command_finished(
    void)
{
#if !defined(_WIN32_WCE)
    if (client_sock != ORTP_PIPE_INVALID)
...
char **convert_args_to_ascii(
    int argc, _TCHAR **wargv)
{
    int  i;
 ...
 */
int
linphonec_prompt_for_auth_final(
    LinphoneCore *lc)
{
...
#ifdef HAVE_READLINE
static char **
linephonec_readline_completion(
